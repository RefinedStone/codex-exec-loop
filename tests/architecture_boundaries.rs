...
fn shell_chrome_state_has_one_typed_reducer_writer() {
    ...
    known_function_returns.materialize_trait_defaults(&known_type_aliases);
...
"#;
    ...
    let generic_impl_returns = shell_chrome_writer_audit(
        "struct GenericFactory<T>(T);\n\
         impl<T> GenericFactory<T> {\n\
             fn make(self) -> T { self.0 }\n\
             fn make_static() -> T { todo!() }\n\
         }\n\
         fn escape(factory: GenericFactory<ShellChromeState>) {\n\
             factory.make().session_state = SessionState::Idle;\n\
             GenericFactory::<ShellChromeState>::make_static().session_state =\n\
                 SessionState::Idle;\n\
         }",
    )
    .expect("generic impl return fixtures should parse");
    assert_eq!(
        generic_impl_returns.field_writes.len(),
        2,
        "impl receiver arguments must instantiate method and associated-function returns"
    );
    let function_turbofish_return = shell_chrome_writer_audit(
        "fn make<T>() -> T { todo!() }\n\
         fn escape() {\n\
             make::<ShellChromeState>().session_state = SessionState::Idle;\n\
         }\n\
         fn update() {\n\
             make::<OtherState>().session_state = 1;\n\
         }",
    )
    .expect("function turbofish return fixture should parse");
    assert_eq!(
        function_turbofish_return.field_writes.len(),
        1,
        "free-function turbofish arguments must instantiate generic returns"
    );
    let method_turbofish_return = shell_chrome_writer_audit(
        "struct GenericMethodFactory;\n\
         impl GenericMethodFactory {\n\
             fn make<T>(&self) -> T { todo!() }\n\
         }\n\
         fn escape(factory: GenericMethodFactory) {\n\
             factory.make::<ShellChromeState>().session_state = SessionState::Idle;\n\
             factory.make::<OtherState>().session_state = 1;\n\
         }",
    )
    .expect("method turbofish return fixture should parse");
    assert_eq!(
        method_turbofish_return.field_writes.len(),
        1,
        "method turbofish arguments must instantiate generic returns"
    );
    let trait_ufcs_returns = shell_chrome_writer_audit(
        "struct UfcsFactory;\n\
         trait ChromeMaker {\n\
             fn make(&self) -> ShellChromeState;\n\
         }\n\
         impl ChromeMaker for UfcsFactory {\n\
             fn make(&self) -> ShellChromeState { todo!() }\n\
         }\n\
         fn escape(factory: UfcsFactory) {\n\
             ChromeMaker::make(&factory).session_state = SessionState::Idle;\n\
             <UfcsFactory as ChromeMaker>::make(&factory).session_state =\n\
                 SessionState::Idle;\n\
         }",
    )
    .expect("trait UFCS return fixtures should parse");
    assert_eq!(
        trait_ufcs_returns.field_writes.len(),
        2,
    ...
    let owned_associated_projections = shell_chrome_writer_audit(
        "trait BaseMaker {\n\
             type Output;\n\
         }\n\
         trait DerivedMaker: BaseMaker {\n\
             type Output;\n\
             fn make(&self) -> <Self as BaseMaker>::Output;\n\
         }\n\
         struct ChromeProjectionFactory;\n\
         impl BaseMaker for ChromeProjectionFactory {\n\
             type Output = ShellChromeState;\n\
         }\n\
         impl DerivedMaker for ChromeProjectionFactory {\n\
             type Output = OtherState;\n\
             fn make(&self) -> <Self as BaseMaker>::Output { todo!() }\n\
         }\n\
         struct OtherProjectionFactory;\n\
         impl BaseMaker for OtherProjectionFactory {\n\
             type Output = OtherState;\n\
         }\n\
         impl DerivedMaker for OtherProjectionFactory {\n\
             type Output = ShellChromeState;\n\
             fn make(&self) -> <Self as BaseMaker>::Output { todo!() }\n\
         }\n\
         fn update(chrome: ChromeProjectionFactory, other: OtherProjectionFactory) {\n\
             chrome.make().session_state = SessionState::Idle;\n\
             other.make().session_state = 1;\n\
         }",
    )
    .expect("trait-owned associated projection fixtures should parse");
    assert_eq!(
        owned_associated_projections.field_writes.len(),
        1,
    ...
    let trait_instance_associated_projection = shell_chrome_writer_audit(
        "trait InstanceBase<T> {\n\
             type Output;\n\
         }\n\
         trait InstanceDerived: InstanceBase<u8> {\n\
             fn make(&self) -> <Self as InstanceBase<u8>>::Output;\n\
         }\n\
         struct MultiBaseFactory;\n\
         impl InstanceBase<u8> for MultiBaseFactory {\n\
             type Output = ShellChromeState;\n\
         }\n\
         impl InstanceBase<u16> for MultiBaseFactory {\n\
             type Output = OtherState;\n\
         }\n\
         impl InstanceDerived for MultiBaseFactory {\n\
             fn make(&self) -> <Self as InstanceBase<u8>>::Output { todo!() }\n\
         }\n\
         fn escape(factory: MultiBaseFactory) {\n\
             factory.make().session_state = SessionState::Idle;\n\
         }",
    )
    .expect("trait-instance associated projection fixture should parse");
    assert_eq!(
        trait_instance_associated_projection.field_writes.len(),
        1,
        "associated projection resolution must select the requested generic trait instance"
    );
    let generic_inherited_associated_projection = shell_chrome_writer_audit(
        "trait GenericOwnerBase<T> {\n\
             type Output;\n\
         }\n\
         trait GenericOwnerDerived<T>: GenericOwnerBase<T> {\n\
             fn make(&self) -> Self::Output;\n\
         }\n\
         struct GenericOwnerFactory<T>(T);\n\
         impl<T> GenericOwnerBase<T> for GenericOwnerFactory<T> {\n\
             type Output = T;\n\
         }\n\
         impl<T> GenericOwnerDerived<T> for GenericOwnerFactory<T> {\n\
             fn make(&self) -> Self::Output { todo!() }\n\
         }\n\
         fn update(\n\
             chrome: GenericOwnerFactory<ShellChromeState>,\n\
             other: GenericOwnerFactory<OtherState>,\n\
         ) {\n\
             chrome.make().session_state = SessionState::Idle;\n\
             other.make().session_state = 1;\n\
         }",
    )
    .expect("generic inherited associated projection fixture should parse");
    assert_eq!(
        generic_inherited_associated_projection.field_writes.len(),
        1,
    ...
    let generic_associated_type_projection = shell_chrome_writer_audit(
        "trait GatMaker {\n\
             type Output<T>;\n\
             fn make<T>(&self) -> Self::Output<T>;\n\
         }\n\
         struct GatFactory;\n\
         impl GatMaker for GatFactory {\n\
             type Output<T> = T;\n\
             fn make<T>(&self) -> Self::Output<T> { todo!() }\n\
         }\n\
         fn update(factory: GatFactory) {\n\
             factory.make::<ShellChromeState>().session_state = SessionState::Idle;\n\
             factory.make::<OtherState>().session_state = 1;\n\
         }",
    )
    .expect("generic associated type projection fixture should parse");
    assert_eq!(
    ...
    let cross_file_reexport = syn::parse_file("pub use crate::traits::CrossFileMaker;")
        .expect("cross-file trait re-export fixture should parse");
    let cross_file_impl = syn::parse_file(
        "pub struct CrossFileFactory;\n\
         impl crate::api::CrossFileMaker for CrossFileFactory {\n\
             type Output = ShellChromeState;\n\
         }",
    )
    .expect("cross-file default impl fixture should parse");
    let trait_module = ["crate".to_string(), "traits".to_string()];
    let api_module = ["crate".to_string(), "api".to_string()];
    ...
    extend_qualified_function_returns(
        &mut cross_file_returns,
        qualified_function_returns_declared_in_file(&cross_file_reexport, &api_module),
    );
    extend_qualified_function_returns(
        &mut cross_file_returns,
    ...
    assert_eq!(
        ...
        1,
        "re-exported trait defaults and associated types must materialize across source files"
    );
    let const_impl_applicability = shell_chrome_writer_audit(
        "struct ConstFactory<const N: usize>;\n\
         impl ConstFactory<1> {\n\
             fn make(&self) -> OtherState { todo!() }\n\
             fn make_static() -> OtherState { todo!() }\n\
         }\n\
         impl ConstFactory<2> {\n\
             fn make(&self) -> ShellChromeState { todo!() }\n\
             fn make_static() -> ShellChromeState { todo!() }\n\
         }\n\
         fn escape(factory: ConstFactory<2>) {\n\
             factory.make().session_state = SessionState::Idle;\n\
             ConstFactory::<2>::make_static().session_state = SessionState::Idle;\n\
         }\n\
         fn update(factory: ConstFactory<1>) {\n\
             factory.make().session_state = 1;\n\
             ConstFactory::<1>::make_static().session_state = 1;\n\
         }",
    )
    .expect("const impl applicability fixtures should parse");
    assert_eq!(
        const_impl_applicability.field_writes.len(),
        2,
        "const generic arguments must select only the applicable impl"
    );
                ...
                .map(|argument| match argument {
                    ...
                    syn::GenericArgument::AssocType(association) => {
                        format!(
                            "{}{}={}",
                            association.ident,
                            association.generics.to_token_stream(),
                    ...
                    syn::GenericArgument::AssocConst(association) => {
                        format!(
                            ...
                            association.ident,
                            association.generics.to_token_stream(),
                            association.value.to_token_stream()
                        )
                    }
                    syn::GenericArgument::Constraint(constraint) => {
                        format!(
                            "{}{}:{}",
                            constraint.ident,
                            constraint.generics.to_token_stream(),
                            constraint.bounds.to_token_stream()
                        )
                    }
                    _ => argument.to_token_stream().to_string(),
                })
                .collect::<Vec<_>>()
                .join(",");
            format!("<{arguments}>")
...
struct ShellGenericSubstituter<'a> {
...
impl VisitMut for ShellGenericSubstituter<'_> {
    ...
    fn visit_expr_mut(&mut self, expression: &mut syn::Expr) {
        if let Some(name) = shell_simple_const_name(expression)
            && let Some(replacement) = self.bindings.consts.get(&name)
        {
...
enum ShellDeclaredGenericParameter {
    Lifetime(String),
    Type(String),
    Const(String),
}
    ...
    generics
        ...
        .map(|parameter| match parameter {
            syn::GenericParam::Lifetime(parameter) => {
                ShellDeclaredGenericParameter::Lifetime(parameter.lifetime.ident.to_string())
            }
            syn::GenericParam::Type(parameter) => {
                ShellDeclaredGenericParameter::Type(parameter.ident.to_string())
            }
            syn::GenericParam::Const(parameter) => {
                ShellDeclaredGenericParameter::Const(parameter.ident.to_string())
            }
        })
        .collect()
}
...
impl ShellGenericBindings {
    fn merge(&self, additional: &Self) -> Option<Self> {
        let mut merged = self.clone();
        for (name, ty) in &additional.types {
        ...
        for (name, expression) in &additional.consts {
            if let Some(existing) = merged.consts.get(name)
                && existing.to_token_stream().to_string()
                    != expression.to_token_stream().to_string()
            {
                return None;
            }
            merged.consts.insert(name.clone(), expression.clone());
        }
        for (name, lifetime) in &additional.lifetimes {
            if let Some(existing) = merged.lifetimes.get(name)
                && existing.ident != lifetime.ident
            {
                return None;
            }
        ...
        match (&merged.current_trait, &additional.current_trait) {
            ...
            (None, Some(additional)) => {
                merged.current_trait = Some(additional.clone());
            }
            _ => {}
        }
    ...
    fn associated_type(&self, key: &ShellAssociatedTypeKey) -> Option<&syn::Type> {
        ...
        loop {
            ...
            if let Some(owner) = self.associated_type_owners.get(current) {
                current = owner;
                continue;
            }
            let template = ShellAssociatedTypeKey {
...
fn shell_generic_parameter_is_type(
    parameters: &[ShellDeclaredGenericParameter],
    name: &str,
) -> bool {
    parameters.iter().any(
        |parameter| matches!(parameter, ShellDeclaredGenericParameter::Type(parameter) if parameter == name),
    )
}

fn shell_generic_parameter_is_const(
    parameters: &[ShellDeclaredGenericParameter],
    name: &str,
) -> bool {
    parameters.iter().any(
        |parameter| matches!(parameter, ShellDeclaredGenericParameter::Const(parameter) if parameter == name),
    )
}

fn shell_simple_type_name(ty: &syn::Type) -> Option<String> {
...
fn shell_const_expression_identity(expression: &syn::Expr) -> String {
    expression.to_token_stream().to_string()
}

fn shell_bind_const_pattern(
    parameter: &str,
    actual: &syn::Expr,
    bindings: &mut ShellGenericBindings,
) -> bool {
    if let Some(bound) = bindings.consts.get(parameter) {
        shell_const_expression_identity(bound) == shell_const_expression_identity(actual)
    } else {
        bindings
            .consts
            .insert(parameter.to_string(), actual.clone());
        true
    }
}

fn shell_const_pattern_matches(
    expected: &syn::Expr,
    actual: &syn::Expr,
    parameters: &[ShellDeclaredGenericParameter],
    bindings: &mut ShellGenericBindings,
) -> bool {
    if let Some(parameter) = shell_simple_const_name(expected)
        && shell_generic_parameter_is_const(parameters, &parameter)
    {
        return shell_bind_const_pattern(&parameter, actual, bindings);
    }
...
fn shell_type_pattern_matches(
    ...
    if let Some(parameter) = shell_simple_type_name(expected)
        && shell_generic_parameter_is_type(parameters, &parameter)
    {
        if let Some(bound) = bindings.types.get(&parameter) {
            return shell_type_identity(bound) == shell_type_identity(actual);
        }
        bindings.types.insert(parameter, actual.clone());
        return true;
    }
    match (expected, actual) {
        (syn::Type::Group(expected), _) => {
            shell_type_pattern_matches(expected.elem.as_ref(), actual, parameters, bindings)
        }
        (_, syn::Type::Group(actual)) => {
            shell_type_pattern_matches(expected, actual.elem.as_ref(), parameters, bindings)
        }
                ...
                && expected
                    ...
                    .all(|(expected, actual)| {
                        expected.ident == actual.ident
                            && shell_path_arguments_pattern_matches(
                                &expected.arguments,
                                &actual.arguments,
                                parameters,
                                bindings,
        ...
        (syn::Type::Slice(expected), syn::Type::Slice(actual)) => shell_type_pattern_matches(
            expected.elem.as_ref(),
            actual.elem.as_ref(),
            parameters,
            bindings,
        ),
        (syn::Type::Array(expected), syn::Type::Array(actual)) => {
            shell_type_pattern_matches(
                expected.elem.as_ref(),
                actual.elem.as_ref(),
                parameters,
                bindings,
            ) && shell_const_pattern_matches(&expected.len, &actual.len, parameters, bindings)
        }
        (syn::Type::Tuple(expected), syn::Type::Tuple(actual)) => {
            expected.elems.len() == actual.elems.len()
                && expected
                    ...
                    .zip(&actual.elems)
                    .all(|(expected, actual)| {
                        shell_type_pattern_matches(expected, actual, parameters, bindings)
                    })
        }
...
fn shell_generic_argument_pattern_matches(
    expected: &syn::GenericArgument,
    actual: &syn::GenericArgument,
    parameters: &[ShellDeclaredGenericParameter],
    bindings: &mut ShellGenericBindings,
) -> bool {
    match (expected, actual) {
        (syn::GenericArgument::Lifetime(expected), syn::GenericArgument::Lifetime(actual)) => {
            let name = expected.ident.to_string();
            let is_parameter = parameters.iter().any(|parameter| {
                matches!(parameter, ShellDeclaredGenericParameter::Lifetime(parameter) if parameter == &name)
            });
        ...
        (syn::GenericArgument::Type(expected), syn::GenericArgument::Type(actual)) => {
            if let Some(parameter) = shell_simple_type_name(expected)
                && shell_generic_parameter_is_const(parameters, &parameter)
            {
                let Ok(actual) = syn::parse_str::<syn::Expr>(&shell_type_identity(actual)) else {
                    return false;
                };
                return shell_bind_const_pattern(&parameter, &actual, bindings);
            }
        ...
        (syn::GenericArgument::Type(expected), syn::GenericArgument::Const(actual))
            if shell_simple_type_name(expected)
                .is_some_and(|name| shell_generic_parameter_is_const(parameters, &name)) =>
        {
            let parameter =
                shell_simple_type_name(expected).expect("a checked const parameter must exist");
            shell_bind_const_pattern(&parameter, actual, bindings)
        }
        (syn::GenericArgument::Const(expected), syn::GenericArgument::Const(actual)) => {
            shell_const_pattern_matches(expected, actual, parameters, bindings)
        }
        (syn::GenericArgument::AssocType(expected), syn::GenericArgument::AssocType(actual)) => {
            expected.ident == actual.ident
                && expected.generics.to_token_stream().to_string()
                    == actual.generics.to_token_stream().to_string()
                && shell_type_pattern_matches(&expected.ty, &actual.ty, parameters, bindings)
        }
        (syn::GenericArgument::AssocConst(expected), syn::GenericArgument::AssocConst(actual)) => {
            expected.ident == actual.ident
                && expected.generics.to_token_stream().to_string()
                    == actual.generics.to_token_stream().to_string()
                && shell_const_pattern_matches(&expected.value, &actual.value, parameters, bindings)
        }
        (syn::GenericArgument::Constraint(expected), syn::GenericArgument::Constraint(actual)) => {
            expected.ident == actual.ident
                && expected.to_token_stream().to_string() == actual.to_token_stream().to_string()
        }
        _ => expected.to_token_stream().to_string() == actual.to_token_stream().to_string(),
    }
}

fn shell_path_arguments_pattern_matches(
    ...
    match (expected, actual) {
        ...
        (
            ...
            expected.args.len() == actual.args.len()
                && expected
                    .args
                    .iter()
                    .zip(&actual.args)
                    .all(|(expected, actual)| {
                        shell_generic_argument_pattern_matches(
                            expected, actual, parameters, bindings,
                        )
                    })
        }
        (
            syn::PathArguments::Parenthesized(expected),
            syn::PathArguments::Parenthesized(actual),
        ) => {
            expected.inputs.len() == actual.inputs.len()
                && expected
                    .inputs
                    .iter()
                    .zip(&actual.inputs)
                ...
                && match (&expected.output, &actual.output) {
                    ...
                    (syn::ReturnType::Type(_, expected), syn::ReturnType::Type(_, actual)) => {
                        shell_type_pattern_matches(
                            expected.as_ref(),
                            actual.as_ref(),
                            parameters,
...
fn shell_explicit_generic_bindings(
    parameters: &[ShellDeclaredGenericParameter],
    ...
    for argument in &arguments.args {
        loop {
            ...
            match (parameter, argument) {
                (
                    ShellDeclaredGenericParameter::Lifetime(name),
                    syn::GenericArgument::Lifetime(lifetime),
                ) => {
                    bindings.lifetimes.insert(name.clone(), lifetime.clone());
                    break;
                }
                (ShellDeclaredGenericParameter::Lifetime(_), _) => continue,
                (ShellDeclaredGenericParameter::Type(name), syn::GenericArgument::Type(ty)) => {
                    bindings.types.insert(name.clone(), ty.clone());
                    break;
                }
                (
                    ...
                    bindings.consts.insert(name.clone(), expression.clone());
                    break;
                }
                (ShellDeclaredGenericParameter::Const(name), syn::GenericArgument::Type(ty)) => {
                    let expression = syn::parse_str::<syn::Expr>(&shell_type_identity(ty)).ok()?;
                    bindings.consts.insert(name.clone(), expression);
                    break;
                }
                _ => return None,
            }
...
struct ShellReturnInstantiation<'a> {
    ...
    trait_instance: Option<String>,
    associated_types: Option<&'a HashMap<ShellAssociatedTypeKey, ShellDeclaredAssociatedType>>,
...
struct ShellQualifiedFunctionReturn {
    ...
    association_receiver_key: Option<String>,
    ...
    associated_types: HashMap<ShellAssociatedTypeKey, ShellDeclaredAssociatedType>,
}

#[derive(Clone, Debug, Eq, Hash, PartialEq)]
struct ShellAssociatedTypeKey {
...
#[derive(Clone)]
struct ShellCanonicalTraitReference {
    identity: String,
    instance: String,
}
...
struct ShellTraitImplementation {
    trait_identity: String,
    trait_instance: String,
    trait_path: syn::Path,
    receiver_paths: Vec<String>,
    association_receiver_key: String,
    receiver: syn::Type,
    impl_module_path: Vec<String>,
    impl_generic_parameters: Vec<ShellDeclaredGenericParameter>,
    associated_types: HashMap<ShellAssociatedTypeKey, ShellDeclaredAssociatedType>,
...
fn canonical_trait_type(
    path: &syn::Path,
    module_path: &[String],
    aliases: &ShellQualifiedTypeAliases,
) -> syn::Type {
    let local_aliases = ShellTypeAliases::new();
    let scope = ShellTypeResolutionScope {
        implicit_self: None,
        type_aliases: &local_aliases,
        qualified_type_aliases: aliases,
        imports: aliases.imports_for_module(module_path),
        absolute_imports: aliases.absolute_imports_for_module(module_path),
        path_shadows: aliases.path_shadows_for_module(module_path),
        module_path,
        allow_local_aliases: false,
        allow_local_imports: true,
    };
    ...
    ty = shell_type_with_expanded_aliases(&ty, scope, &mut HashSet::new());
    ShellReturnTypeQualifier { scope }.visit_type_mut(&mut ty);
    ty
}

fn canonical_trait_identity(
    path: &syn::Path,
    module_path: &[String],
    aliases: &ShellQualifiedTypeAliases,
) -> String {
    let ty = canonical_trait_type(path, module_path, aliases);
    shell_type_path(&ty)
        .map(|path| {
            path.path
                .segments
        ...
        .unwrap_or_else(|| declared_shell_path(path, module_path).join("::"))
}

fn canonical_trait_instance_identity(
    path: &syn::Path,
    module_path: &[String],
    aliases: &ShellQualifiedTypeAliases,
) -> String {
    let ty = canonical_trait_type(path, module_path, aliases);
    shell_type_identity(&ty)
}

fn shell_associated_type_projection(
    ty: &syn::Type,
    scope: ShellTypeResolutionScope<'_>,
    current_trait: Option<&str>,
) -> Option<ShellAssociatedTypeProjection> {
    let syn::Type::Path(path) = ty else {
        return None;
    };
    let output = path.path.segments.last()?;
    let arguments_identity = shell_path_arguments_identity(&output.arguments);
    ...
    if path.qself.is_none()
        ...
        && path.path.segments[0].ident == "Self"
    {
        return Some(ShellAssociatedTypeProjection {
            key: ShellAssociatedTypeKey {
...
fn shell_associated_type_projection_key(
    ty: &syn::Type,
    scope: ShellTypeResolutionScope<'_>,
    current_trait: Option<&str>,
) -> Option<ShellAssociatedTypeKey> {
    shell_associated_type_projection(ty, scope, current_trait).map(|projection| projection.key)
}

fn shell_associated_type_projections(
    ty: &syn::Type,
    scope: ShellTypeResolutionScope<'_>,
    current_trait: Option<&str>,
) -> Vec<ShellAssociatedTypeProjection> {
    struct ProjectionCollector<'a> {
        scope: ShellTypeResolutionScope<'a>,
        current_trait: Option<&'a str>,
        seen: HashSet<ShellAssociatedTypeKey>,
        projections: Vec<ShellAssociatedTypeProjection>,
    }

    impl<'ast> Visit<'ast> for ProjectionCollector<'_> {
        fn visit_type_path(&mut self, path: &'ast syn::TypePath) {
            let ty = syn::Type::Path(path.clone());
            if let Some(projection) =
                shell_associated_type_projection(&ty, self.scope, self.current_trait)
                && self.seen.insert(projection.key.clone())
            {
    ...
    let mut collector = ProjectionCollector {
        scope,
        current_trait,
        seen: HashSet::new(),
        projections: Vec::new(),
...
fn shell_associated_type_owner_key(
    key: &ShellAssociatedTypeKey,
    owners: &HashMap<ShellAssociatedTypeKey, ShellAssociatedTypeKey>,
    ...
    while visited.insert(current.clone()) {
        ...
        current = ShellAssociatedTypeKey {
            trait_identity: owner.trait_identity.clone(),
            name: owner.name.clone(),
...
impl VisitMut for ShellAssociatedTypeSubstituter<'_> {
    fn visit_type_mut(&mut self, ty: &mut syn::Type) {
        let mut instantiated_projection = ty.clone();
        let mut projection_bindings = self.bindings.clone();
        projection_bindings.types.remove("Self");
        ShellGenericSubstituter {
            bindings: &projection_bindings,
        }
        .visit_type_mut(&mut instantiated_projection);
        if let Some(key) = shell_associated_type_projection_key(
            &instantiated_projection,
            self.scope,
...
fn shell_trait_associated_owners(
    ...
    supertraits: &HashMap<String, Vec<ShellCanonicalTraitReference>>,
    ...
    let mut owners = definitions
        ...
        .map(|definition| {
            definition
                ...
                .iter()
                .map(|name| (name.clone(), trait_instance.to_string()))
                .collect::<HashMap<_, _>>()
        })
        .unwrap_or_default();
    let mut inherited = supertraits.get(trait_identity).cloned().unwrap_or_default();
    inherited.sort_by(|left, right| left.instance.cmp(&right.instance));
...
impl ShellQualifiedFunctionReturns {
    ...
    fn materialize_trait_defaults(&mut self, aliases: &ShellQualifiedTypeAliases) {
            ...
            .map(|(trait_identity, definition)| {
                (
                    ...
                    definition
                        .supertraits
                        .iter()
                        .map(|supertrait| ShellCanonicalTraitReference {
                            identity: canonical_trait_identity(
                                &supertrait.path,
                                &supertrait.module_path,
                                aliases,
                            ),
                            instance: canonical_trait_instance_identity(
                                &supertrait.path,
                                &supertrait.module_path,
                                aliases,
                            ),
        ...
        for implementation in &mut implementations {
            implementation.trait_identity = canonical_trait_identity(
                &implementation.trait_path,
                &implementation.impl_module_path,
                aliases,
            );
            implementation.trait_instance = canonical_trait_instance_identity(
                &implementation.trait_path,
                &implementation.impl_module_path,
                aliases,
            );
            implementation.associated_types = implementation
                .associated_types
                .clone()
                .into_iter()
                .map(|(key, associated_type)| {
                    (
                        ShellAssociatedTypeKey {
        ...
        for (key, entries) in &mut self.returns {
            let Some(method) = key.rsplit("::").next() else {
                continue;
            };
            for entry in entries {
                let ShellFunctionReturnSource::Trait(trait_identity) = &mut entry.source else {
                    continue;
                };
                let Some(impl_trait) = &entry.impl_trait else {
                    continue;
                };
                let declared_identity = trait_identity.clone();
                let canonical =
                    canonical_trait_identity(impl_trait, &entry.impl_module_path, aliases);
                let canonical_instance =
                    canonical_trait_instance_identity(impl_trait, &entry.impl_module_path, aliases);
                *trait_identity = canonical.clone();
                entry.associated_types = entry
                    .associated_types
                    .clone()
                    .into_iter()
                    .map(|(key, associated_type)| {
                    ...
                    .collect();
                if key == &format!("{declared_identity}::{method}")
                    && canonical != declared_identity
                {
                    canonical_entries.push((format!("{canonical}::{method}"), entry.clone()));
                }
            }
        }
        for (key, entry) in canonical_entries {
            self.insert_return(key, entry);
        }
        ...
        for implementation in &mut implementations {
            ...
            for default_return in definition.default_returns {
                ...
                let entry = ShellQualifiedFunctionReturn {
                    ...
                    association_receiver_key: Some(implementation.association_receiver_key.clone()),
        ...
        for entries in self.returns.values_mut() {
            for entry in entries {
                ...
                if let Some(associated_types) = associated_by_receiver.get(receiver_key) {
                    entry.associated_types.extend(associated_types.clone());
                }
            }
        }
...
fn collect_item_function_return(item: &syn::Item, returns: &mut ShellFunctionReturns) {
    ...
    if let syn::ReturnType::Type(_, ty) = &function.sig.output {
        returns.insert(
            ...
            ShellFunctionReturn {
                ty: ty.as_ref().clone(),
                generic_parameters: shell_declared_generic_parameters(&function.sig.generics),
            },
        );
    }
}
...
fn declared_shell_type_identity(ty: &syn::Type, module_path: &[String]) -> String {
    let Some(path) = shell_type_path(ty) else {
        return shell_type_identity(ty);
    };
    let arguments = path
        .path
        .segments
        .last()
        .map(|segment| shell_path_arguments_identity(&segment.arguments))
        .unwrap_or_default();
    format!(
        "{}{arguments}",
    ...
    for item in items {
        ...
        match item {
                    ...
                    .filter_map(|trait_item| {
                        ...
                        Some((
                            ...
                            ShellDeclaredAssociatedType {
                                ty: ty.clone(),
                                module_path: module_path.clone(),
                                generic_parameters: shell_declared_generic_parameters(
            ...
            syn::Item::Impl(item) => {
                ...
                let association_receiver_key =
                    declared_shell_type_identity(item.self_ty.as_ref(), module_path);
                let associated_types = trait_identity
                    .as_ref()
                    .map(|trait_identity| {
                        item.items
                            .iter()
                ...
                if let Some(trait_identity) = &trait_identity {
                    ...
                    returns
                        ...
                        .push(ShellTraitImplementation {
                            trait_identity: trait_identity.clone(),
                            trait_instance: shell_type_identity(&syn::Type::Path(syn::TypePath {
                                qself: None,
                                path: trait_path.clone(),
                            })),
                            trait_path,
                            receiver_paths: receiver_paths.clone(),
                            association_receiver_key: association_receiver_key.clone(),
                ...
                registry_owners.sort();
                registry_owners.dedup();
                    ...
                    for receiver in &registry_owners {
                        returns.insert_return(
                            ...
                            ShellQualifiedFunctionReturn {
                                ...
                                association_receiver_key: Some(association_receiver_key.clone()),
                                impl_trait: item
                                    .trait_
                                    .as_ref()
                                    .map(|(_, trait_path, _)| trait_path.clone()),
...
fn shell_chrome_writer_audit_with_registries(
    ...
    if !qualified_function_returns.materialized {
        extend_qualified_function_returns(
            &mut qualified_function_returns,
            qualified_function_returns_declared_in_file(&syntax, module_path),
        );
        qualified_function_returns.materialize_trait_defaults(&qualified_type_aliases);
    }
...
impl ShellChromeWriterVisitor {
    ...
    fn resolved_declared_return_type(
        ...
        self.resolved_declared_return_type_with_bindings(
            ty,
            declaration_module,
            receiver,
    ...
    fn resolved_declared_return_type_with_bindings(
        &self,
        ty: &syn::Type,
        declaration_module: &[String],
        receiver: Option<&ShellTypeBinding>,
    ...
    fn associated_type_owner_instance(
        &self,
        trait_instance: &str,
        associated_name: &str,
        visiting: &mut HashSet<String>,
    ) -> Option<String> {
        if !visiting.insert(trait_instance.to_string()) {
            return None;
        }
        let ty = syn::parse_str::<syn::Type>(trait_instance).ok()?;
        let path = shell_type_path(&ty)?;
        ...
        let definition = self
            ...
            .trait_definitions
            .get(&trait_identity)?;
        if definition.associated_type_names.contains(associated_name) {
            return Some(trait_instance.to_string());
        }
        let arguments = path.path.segments.last().and_then(|segment| {
            let syn::PathArguments::AngleBracketed(arguments) = &segment.arguments else {
                return None;
            };
            Some(arguments)
        ...
        for supertrait in &definition.supertraits {
            ...
            let supertrait_instance = shell_type_identity(&resolved);
            if let Some(owner) =
                self.associated_type_owner_instance(&supertrait_instance, associated_name, visiting)
            {
                return Some(owner);
            }
        }
    ...
    fn associated_type_owner_key(&self, key: &ShellAssociatedTypeKey) -> ShellAssociatedTypeKey {
        let mut visiting = HashSet::new();
        if let Some(trait_identity) =
            self.associated_type_owner_instance(&key.trait_identity, &key.name, &mut visiting)
        {
            return ShellAssociatedTypeKey {
                trait_identity,
                name: key.name.clone(),
        ...
        shell_associated_type_owner_key(
            key,
            &self.qualified_function_returns.associated_type_owners,
        )
    }
    ...
    fn applicable_associated_types(
        ...
        for projection in projections {
            ...
            for implementation in &self.qualified_function_returns.trait_implementations {
                let Some(receiver_bindings) = self.impl_receiver_pattern_matches(
                ...
                let Some(trait_bindings) = self.impl_trait_pattern_matches(
                    &implementation.trait_path,
                    &implementation.impl_module_path,
                    &implementation.impl_generic_parameters,
                    &actual_trait,
                ...
                let Some(associated_type) =
                    implementation
                        .associated_types
                        .iter()
                        .find_map(|(key, associated_type)| {
                            (key.name == owner.name).then_some(associated_type)
                        })
                else {
                    continue;
                };
                let Some(gat_bindings) = shell_explicit_generic_bindings(
                    &associated_type.generic_parameters,
                    projection.arguments.as_ref(),
                ) else {
                    continue;
                };
    ...
    fn impl_trait_matches(
        ...
        self.impl_trait_pattern_matches(
            expected_trait,
            &entry.impl_module_path,
            &entry.impl_generic_parameters,
            actual_trait,
    ...
    fn trait_is_visible(&self, trait_path: &str) -> bool {
        ...
        if self
            ...
            && let Ok(path) = syn::parse_str::<syn::Path>(&local_path)
            && canonical_trait_identity(&path, &self.module_path, &self.qualified_type_aliases)
                == trait_path
        {
            return true;
        }
        ...
        self.qualified_type_aliases
            ...
            .any(|target| {
                let canonical = canonical_trait_identity(
                    &target.path,
                    &self.module_path,
                    &self.qualified_type_aliases,
                );
                canonical == trait_module
            })
    }

    fn impl_trait_instance(
        &self,
        entry: &ShellQualifiedFunctionReturn,
        receiver: &ShellTypeBinding,
        bindings: &ShellGenericBindings,
    ) -> Option<String> {
        let trait_path = entry.impl_trait.as_ref()?;
        let ty = syn::Type::Path(syn::TypePath {
            qself: None,
            path: trait_path.clone(),
        });
        Some(shell_type_identity(
            &self.resolved_declared_return_type_with_bindings(
                &ty,
                &entry.impl_module_path,
                Some(receiver),
    ...
    fn explicit_return_type(
        ...
        bindings.current_trait = instantiation.trait_instance.clone();
        ...
        for (key, associated_type) in instantiation.associated_types.into_iter().flatten() {
            if !associated_type.generic_parameters.is_empty() {
                continue;
            }
        ...
        let declaration_scope = self.return_type_resolution_scope(declaration_module);
        let mut projection_ty = ty.clone();
        ShellGenericSubstituter {
            bindings: &bindings,
        }
        .visit_type_mut(&mut projection_ty);
        projection_ty = shell_type_with_expanded_aliases(
            &projection_ty,
            declaration_scope,
            &mut HashSet::new(),
        );
    ...
    fn associated_call_receiver(&self, path: &syn::ExprPath) -> Option<ShellTypeBinding> {
        if let Some(qself) = &path.qself {
            return Some(ShellTypeBinding {
                ty: qself.ty.as_ref().clone(),
                mutable: false,
            });
        }
        if path.path.segments.len() < 2 {
        ...
        let mut receiver = path.path.clone();
    ...
    fn function_return_type(
        &self,
        path: &syn::ExprPath,
        call_arguments: &syn::punctuated::Punctuated<syn::Expr, syn::Token![,]>,
    ) -> Option<syn::Type> {
        let explicit_arguments = path.path.segments.last().and_then(|segment| {
            let syn::PathArguments::AngleBracketed(arguments) = &segment.arguments else {
                return None;
            };
            Some(arguments)
        ...
        let mut normalized =
            normalized_shell_type_path(&path.path, self.type_resolution_scope(self.impl_authority));
        if normalized.len() == 1 {
            let mut scoped = self.module_path.clone();
            scoped.append(&mut normalized);
            normalized = scoped;
        }
        let owner_path = normalized
        ...
        let associated_receiver = self.associated_call_receiver(path);
        if let Some((entry, bindings)) = returns.iter().find_map(|entry| {
            if !matches!(entry.source, ShellFunctionReturnSource::Inherent) {
                return None;
            }
            let receiver = associated_receiver.as_ref()?;
            self.impl_receiver_matches(entry, receiver)
                .map(|bindings| (entry, bindings))
        }) {
            return self.explicit_return_type(
                &entry.ty,
                &entry.module_path,
        ...
        let mut trait_returns = returns
            ...
            .filter_map(|entry| {
                ...
                let receiver = associated_receiver
                    .as_ref()
                ...
                let trait_instance = self.impl_trait_instance(entry, receiver, &impl_bindings)?;
                let ty = self.explicit_return_type(
                    ...
                    ShellReturnInstantiation {
                        ...
                        trait_instance: Some(trait_instance),
                        associated_types: Some(&entry.associated_types),
                        function_parameters: &entry.function_generic_parameters,
                        arguments: explicit_arguments,
                    },
    ...
    fn method_return_type(
        ...
        receiver: &ShellTypeBinding,
        method: &syn::Ident,
        explicit_arguments: Option<&syn::AngleBracketedGenericArguments>,
    ) -> Option<syn::Type> {
        ...
        let candidates = receiver_keys
            ...
            .collect::<Vec<_>>();
        if let Some((inherent, bindings)) = candidates
            .iter()
            .find(|(entry, _)| matches!(entry.source, ShellFunctionReturnSource::Inherent))
        {
            return self.explicit_return_type(
                &inherent.ty,
        ...
        let mut trait_returns = candidates
            ...
            .filter_map(|(entry, bindings)| {
                ...
                let trait_instance = self.impl_trait_instance(entry, receiver, &bindings)?;
    ...
    fn resolve_type_binding(&self, expression: &syn::Expr) -> Option<ShellTypeBinding> {
        match expression {
            ...
            syn::Expr::MethodCall(call) => {
                ...
                let ty =
                    self.method_return_type(&receiver, &call.method, call.turbofish.as_ref())?;
            ...
            syn::Expr::Call(call) => {
                ...
                let ty = self.function_return_type(path, &call.args)?;
                Some(self.function_return_binding(ty))
            }
            _ => None,
        }
