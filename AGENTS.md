...
- GitHub writes: use the local `gh`/git credential identity through `bash scripts/gh-akra.sh`; set `AKRA_GITHUB_LOGIN=<login>` or `git config akra.githubLogin <login>` when a specific GitHub login is required, and do not write to GitHub if the active identity is uncertain
