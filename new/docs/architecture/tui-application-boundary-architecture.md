...
| State | 현재 위치 예시 | 목표 |
| --- | --- | --- |
| in-flight refresh/wake/tick flags | parallel mode TUI fields | UI spinner만 TUI에 남기고 command correlation은 application runtime store로 이동 |
...
| queued auto prompt metadata | `QueuedAutoPrompt`, `AutoFollowSubmitContext` | conversation lifecycle과 automation lifecycle을 분리한 뒤 application event provenance로 이동 |
