...
projection에서 먼저 읽고, 기존 TUI cache는 전환용 fallback으로만 사용한다.
...
conversation cache는 reducer/automation 판단 호환용으로만 남아 있다.
...
| planning runtime projection | core projection + transitional Ready conversation reducer cache | core app state 또는 application projection | 여러 surface가 같은 planning 상태를 본다. |
| parallel-mode status | application control-plane + core projection + transitional TUI fallback cache | application control-plane + core projection | 기존 single-writer gate를 유지하고 core는 projection만 노출해야 한다. |
