...
용어는 새 slice를 만들 때 아래 의미로만 사용한다.
...
| `AppEvent` | core가 inbound adapter에 알리는 상태 변화 notification | core -> inbound adapter |
...
`Event`를 모든 일에 붙이지 않는다. 외부에서 이미 일어난 사실은 input으로 받아 reducer가
처리하고, reducer 밖으로 알릴 때만 app/runtime event 이름을 쓴다. Background 작업 결과는
`Completed`/`Completion`으로 끝나야 한다.
