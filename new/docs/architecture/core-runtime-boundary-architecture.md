...
용어는 피해야 할 단어 목록이 아니다. 필요한 runtime 개념은 쓰되, 새 slice를 만들 때
아래 의미로 고정해서 같은 개념에 여러 이름을 붙이지 않는다.

| 용어 | 의미 | 위치 |
| --- | --- | --- |
| `Command` | 사용자가 하려는 의도 또는 외부 surface가 보낸 요청 | inbound adapter -> core/application |
| `CoreInput` | core reducer가 받는 envelope. command, external fact, completion을 한 줄로 직렬화한다 | core 내부 |
| `Effect` | reducer가 직접 실행하지 않고 runner에 맡기는 작업 요청 | core/application runtime 내부 |
| `Completion` | effect가 끝난 뒤 같은 runtime으로 재진입하는 결과 | effect runner -> core/application |
| `AppEvent` | core가 inbound adapter에 알리는 app 상태 변화 notification | core -> inbound adapter |
| `Snapshot`/`Projection` | 렌더링/조회용 read model. mutation entrypoint가 아니다 | core/application -> inbound adapter |
| TUI message | terminal adapter 내부 channel payload. app-level event나 domain event로 부르지 않는다 | `adapter/inbound/tui` 내부 |

`Event`는 이미 일어난 사실이나 runtime 밖으로 알리는 notification에 쓴다. 단순히
비동기라는 이유만으로 command/effect/completion/payload를 전부 event로 부르지 않는다.
Background 작업 요청은 `Effect`, 그 결과는 `Completed`/`Completion`으로 끝내고,
adapter channel payload는 `Message`/`Payload`로 남긴다. Stream처럼 domain language가
이미 `ConversationStreamEvent`인 경우에는 그 이름을 유지한다.
...
현재 startup migration은 TUI reducer/rendering state가 `StartupReadySnapshot`을 읽고,
`RunStartupChecks` effect 실행은 `CoreRuntime`/`CoreEffectRunner`가 담당하는 단계까지
...
`SessionCatalog` payload도 보존하고, `LoadSessionCatalog` effect 실행도
`CoreRuntime`/`CoreEffectRunner`를 통과하는 단계까지 진행됐다. TUI는 session overlay
...
`CoreRuntime`/`CoreEffectRunner`를 통과하는 단계까지 진행됐다. `SubmitTurn`,
...
현재 TUI 표시 accessor는 parallel readiness/supervisor를 core `AppSnapshot`
projection에서만 읽고, 별도 TUI write-through cache/fallback field는 없다.
...
| parallel-mode status | application control-plane + core projection + TUI presentation bridge | application control-plane + core projection | 기존 single-writer gate를 유지하고 core는 projection만 노출한다. TUI fallback authority는 두지 않는다. control-plane presentation event 변환은 bridge가 맡고, TUI parallel controller는 presentation action 적용만 맡는다. |
| inline command/overlay/auto-follow routing | TUI presentation routing | TUI presentation routing + application/domain decision | TUI inline command는 diagnostics/session/queue/planning/parallel 같은 표시와 조작 surface만 남긴다. Task 추가는 admin/API와 application task-intake service가 맡고, TUI의 `:task` command/overlay/pending replay 경로는 제거됐다. |
