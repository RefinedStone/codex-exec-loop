...
| inline command/overlay/auto-follow routing | TUI presentation routing + application planning task-intake routing policy | TUI presentation routing + application/domain decision | inline syntax, overlay open/close, editor key routing은 TUI에 남는다. `:task` planning-safe deferral, stale queued-command drop, auto-follow pause 여부는 application planning runtime policy가 소유한다. |
