...
코드 변경 전에 `TUI`, `application single-writer loop`, `domain aggregate`,
...
```text
TUI는 의도를 보낸다.
Application loop는 변경을 한 줄로 직렬화한다.
...
### 2. 모든 변경은 Single-Writer Application Loop를 통과한다
...
application layer에 single-writer loop를 둔다. 이름은 `OrchestratorLoop`, `CommandProcessor`, `ControlPlaneRuntime` 중 무엇이든 가능하지만 역할은 하나다.
...
command/event 수신
  ...
  -> repository/save
  -> effect 실행
  -> completion event enqueue
```

이 loop는 “정책의 주인”이 아니다. 순서, transaction, port 호출, worker launch 같은 실행 책임을 가진다.
...
application loop 안에서 이런 판단이 `if/else`로 커지면 domain이 약해진다. application loop는 domain decision을 실행하는 coordinator여야 한다.
...
### Application Single-Writer Loop
...
- decision 결과 저장
- port effect 실행
- worker/background completion을 다시 command/event로 enqueue

중요한 제약:

- application loop는 domain state를 직접 소유하는 singleton이 아니다.
- application loop는 business rule을 직접 판단하지 않는다.
- application loop는 side effect와 ordering의 주인이다.
...
in-memory repository는 가능하지만 mutation 경로는 single-writer loop로 제한한다.
...
### Phase 3. Application Loop 정리

- `orchestrator_loop.rs`를 command processor 관점으로 정리한다.
...
- process-lifetime 상태가 필요하면 application runtime store로 만들고 single-writer loop만 mutate하게 한다.
...
application loop는 fake repository/port로 직렬 처리 순서를 테스트한다.
