...
| --- | --- | --- | --- |
| TUI planning shell/overlay | `PlanningServices`, planning controllers, status projection, `PlanningResetTarget`, TUI planning shell parser | UI-only overlay/editor state와 application service 호출 분리, `:planning`과 `:reset` execution/hint path는 shared parser로 mapping | 남은 task/editor command request DTO를 admin/CLI와 같은 이름으로 정렬 |
...
4. TUI planning shell command와 admin/CLI reset/status/queue vocabulary의 차이를 문서화한다.
   ...
   vocabulary를 공유한다. 남은 `:task`, editor/overlay command는 공통 application command로
   내릴 수 있는 부분부터 이동한다.
