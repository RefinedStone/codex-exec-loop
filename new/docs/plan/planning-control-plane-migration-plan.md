...
3. TUI/admin/CLI/Telegram 호출부를 한 번에 바꾸지 않고, 먼저 read-only status/queue
   ...
   panel projection `PLAN-01J`, TUI existing-workspace popup projection `PLAN-01K`,
   projection source signature `PLAN-01L`
...
- stable task-id allocation: task mutation과 runtime intake가 중복으로 갖고 있던
