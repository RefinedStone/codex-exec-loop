...
| TUI planning shell/overlay | `:planning`, `:task`, `:queue`, `:directions`, `:reset`, editor/overlay actions | `PlanningServices`, planning controller request mapping, `PlanningResetTarget`, TUI planning/task/overlay shell parsers | `planning_shell_argument_maps_to_tui_planning_command`, `task_shell_argument_maps_to_prompt_intake_command`, `planning_overlay_shell_argument_accepts_only_empty_argument`, `reset_shell_argument_maps_to_shared_application_targets`, `reset_command_hint_is_argument_aware`; 남은 editor vocabulary는 후속 `INBOUND-00D`에서 정렬 |
