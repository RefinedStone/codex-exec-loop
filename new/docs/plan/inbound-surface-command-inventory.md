...
| TUI planning shell/overlay | `:planning`, `:task`, `:queue`, `:directions`, `:reset`, editor/overlay actions | `PlanningServices`, planning controller request mapping, `PlanningResetTarget`, TUI planning/task/overlay shell parsers, TUI draft editor keymap | `planning_shell_argument_maps_to_tui_planning_command`, `task_shell_argument_maps_to_prompt_intake_command`, `planning_overlay_shell_argument_accepts_only_empty_argument`, `reset_shell_argument_maps_to_shared_application_targets`, `reset_command_hint_is_argument_aware`, `editor_overlay_keymaps_stay_tui_local_and_delegate_mutations` |
...
- TUI no-argument planning overlay parser를 `planning_overlay_shell_command` adapter helper로 분리해
  `:queue`/`:directions` 실행과 inline buffered hint가 같은 empty-argument parse result를 사용하게 했다.
- `planning_overlay_shell_argument_accepts_only_empty_argument`, `queue_command_hint_is_argument_aware`,
  `directions_command_hint_is_argument_aware`가 overlay commands의 unsupported argument recovery copy를 고정한다.

## INBOUND-00D-F 완료 근거

- TUI draft editor의 cursor, file selection, text edit, close confirmation keymap은
  `handle_draft_editor_key`와 overlay-specific close confirmation handler 안에 남긴다.
- planning-init manual editor와 directions maintenance editor는 같은 keymap을 쓰되
  `save_*_manual_editor`와 `promote_*_manual_editor` hook만 주입해 mutation을 `PlanningServices`
  workspace service로 위임한다.
- `editor_overlay_keymaps_stay_tui_local_and_delegate_mutations`가 editor keymap에
  `PlanningAdmin*`, `PlanningControl*`, parallel orchestrator command vocabulary가 섞이지 않도록 고정한다.
- editor keymap은 buffer/cursor/close guard를 다루는 TUI focus vocabulary이므로 CLI/admin 공통
  application command로 올리지 않는다. 공통화 대상은 save/promote 결과와 validation projection이다.

## INBOUND-00D 완료 근거

- TUI `:reset`, `:planning`, `:task`, `:queue`, `:directions` shell grammar는 execution path와
  inline hint path가 같은 parser를 공유한다.
- editor/overlay key vocabulary는 application command로 승격하지 않는 UI-only 영역과
  `PlanningServices` mutation delegation 영역으로 분리해 source-level regression으로 고정했다.
