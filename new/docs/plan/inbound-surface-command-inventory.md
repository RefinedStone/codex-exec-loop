...
| TUI planning shell/overlay | `:planning`, `:task`, `:reset`, editor/overlay actions | `PlanningServices`, planning controller request mapping, `PlanningResetTarget`, TUI planning shell parser | `planning_shell_argument_maps_to_tui_planning_command`, `reset_shell_argument_maps_to_shared_application_targets`, `reset_command_hint_is_argument_aware`; 남은 task/editor vocabulary는 후속 `INBOUND-00D`에서 정렬 |
