...
적용한다. Inline `:task` command의 planning-safe deferral, stale queued-command drop,
auto-follow pause 여부는 application planning runtime routing policy가 결정하고 TUI는
command syntax와 presentation effect만 적용한다.
