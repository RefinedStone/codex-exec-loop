...
| Slice | 상태 | 목표 |
| --- | --- | --- |
| TUI-THIN-ROUTING-05 | next | inline command, overlay, auto-follow controls 중 application/domain policy가 섞인 routing을 재분류한다. TUI-only routing은 유지하고 business decision은 application/domain으로 이동한다. |
