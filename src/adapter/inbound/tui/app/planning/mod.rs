...
// debug_panel_state는 planning worker panel의 접힘/상태/가시성 정보를 보관하는 작은 UI state model이다.
...
pub(super) use debug_panel_state::{
    PlanningWorkerPanelState, PlanningWorkerStatus, PlanningWorkerVisibility,
};
// planning worker panel line builder는 shell rendering 쪽에서 필요하므로 상위 planning module 표면으로 올린다.
pub(super) use presentation::build_planning_worker_panel_lines;
