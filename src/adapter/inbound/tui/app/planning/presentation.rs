...
// Planner debug panel은 runtime worker를 제어하지 않고 마지막 관측 snapshot만 읽는 presentation surface다.
...
pub(crate) fn build_planner_panel_lines(app: &NativeTuiApp, max_detail_len: usize) -> Vec<String> {
    // planner worker detail은 operator가 켠 경우에만 노출된다. 기본 shell 화면은 worker internals 대신
    // user-facing planning status surfaces를 우선한다.
    if !app.planner_shows_debug_details() {
        return Vec::new();
    }

    // worker panel state는 post-turn/planner runtime이 갱신한 last-observed snapshot이고, 이 boundary는 읽기만 한다.
    ...
    // notice는 진행 설명보다 operator attention이 필요한 planner-side diagnostic이라 별도 label로 분리한다.
    ...
    // rejected summary는 정상 summary와 섞지 않아 planner가 candidate를 버린 이유를 바로 찾게 한다.
