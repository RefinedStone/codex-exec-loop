...
use super::status_projection::{
    build_planning_followup_surface_projection, build_queue_framing_lines,
...
pub(crate) fn build_automation_status_lines(app: &NativeTuiApp) -> Vec<Line<'static>> {
    match &app.conversation_state {
        ...
        ConversationState::Ready(conversation) => {
            ...
            lines.extend(planning_projection.queue_framing_lines);
            ...
            if let Some(repair_attempt_line) = planning_projection.repair_attempt_line {
                lines.push(Line::from(repair_attempt_line));
            }
            if let Some(failure_line) = planning_projection.failure_line {
