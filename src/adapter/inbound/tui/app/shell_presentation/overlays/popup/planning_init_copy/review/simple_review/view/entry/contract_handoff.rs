...
// handoff index는 copy->contract와 contract->view를 한 namespace에 묶는다.
...
mod contract;
// wiring module은 contract 생성과 final assembly를 실제 순서로 호출한다. handoff index는 하위 단계들을
// 숨기고 copy 입력 하나를 받는 facade를 제공한다.
#[path = "contract_handoff/wiring.rs"]
mod wiring;

// handoff의 최종 산출물도 공통 overlay view다. 이 타입이 상위 popup renderer와 simple review 내부 조립
// pipeline을 연결한다.
...
// handoff는 이 값을 받아 view 생성 pipeline 전체를 실행한다.
use super::super::super::super::super::super::copy::PlanningSimpleReviewCopy;

// `build_simple_review_overlay_view_from_copy`는 handoff 계층의 공개 함수다. caller는 copy만 넘기고,
...
pub(super) fn build_simple_review_overlay_view_from_copy(
    ...
    // wiring module에 실제 순서 결정을 맡겨, 이 파일은 handoff namespace와 public entry 이름을 제공하는
    // 얇은 facade로 남는다.
