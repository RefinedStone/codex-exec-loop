// entry 단계는 simple review view build 요청을 contract handoff pipeline으로 넘긴다.
// path attribute는 `entry/contract_handoff.rs`를 이 entry module의 내부 구현으로 연결한다.
...
// copy는 simple review popup의 text/option 상태 입력이다. entry는 이 값을 받아 contract handoff 계층에
// 넘기는 첫 함수다.
...
// 함수 이름은 최종 산출물을 말하지만, 실제 단계 분리는 아래 contract_handoff module에 맡긴다.
pub(super) fn build_simple_review_overlay_view(
    // `copy` ownership을 넘겨 handoff pipeline이 section/contract/view 생성 과정에서 필요한 데이터를
    // 안전하게 소유하거나 빌릴 수 있게 한다.
    copy: PlanningSimpleReviewCopy,
) -> PlanningInitOverlayView {
    // copy에서 바로 view를 만들지 않고 handoff layer를 거치게 해, copy-to-contract와 contract-to-view
