...
// 출력은 section 수집이 끝난 assembly contract다. 이 contract는 다음 handoff 단계에서 final overlay view로
// 변환된다.
use super::super::super::assembly_contract::PlanningSimpleReviewAssemblyContract;
// chaining helper가 실제 `copy -> sections -> contract` 순서를 수행한다. 이 파일은 contract handoff
// namespace에서 그 helper를 안정적인 이름으로 노출한다.
use super::super::super::chaining::build_simple_review_assembly_contract_for_copy;

// `build_simple_review_assembly_contract_from_copy`는 handoff pipeline의 첫 번째 단계다.
