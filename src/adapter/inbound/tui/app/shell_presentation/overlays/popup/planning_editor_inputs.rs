...
/*
 * Adapter boundary from editor session/runtime facts into compact status-copy input.
 * The surface builder has already selected a session buffer and interpreted runtime close/next-action state;
 * this function preserves those decisions while adding only presentation-sized summaries.
 */
...
pub(super) fn build_planning_draft_editor_status_copy<'a>(
    ...
    PlanningDraftEditorStatusCopy {
        ...
        file_count,
        // Coarse validation state drives status tone; representative issue detail stays separate.
...
// Project the first domain validation issue without re-sorting severity or kind.
...
// Summarize dirty file labels as display text while keeping the boolean signal separate in the DTO.
...
// Tests pin projection policy without booting the editor runtime or renderer.
#[cfg(test)]
mod tests {
    ...
    // Clean dirty-label state should render as explicit "none", not as missing copy.
    #[test]
    fn dirty_label_summary_reports_none_when_clean() {
        assert_eq!(summarize_dirty_labels(&[]), "none");
    }

    // The first validation issue is a domain/report-order decision, not a severity sort in presentation.
    #[test]
