...
pub(crate) fn build_help_overlay_view(language: TuiLanguage) -> HelpOverlayView {
    let entries = InlineShellCommand::help_entries(language);
    ...
    HelpOverlayView {
        header_lines: vec![
            AkraTheme::title_line(
                language.shell_command_help_title(),
                language.shell_command_help_context(),
            ),
            Line::from(language.shell_command_help_intro()),
        ],
        ...
        command_lines: entries
            ...
            .collect(),
        key_lines: vec![AkraTheme::key_line(language.shell_command_help_keys())],
    }
}
