...
mod tests {
    ...
    #[test]
    fn runtime_notice_markers_match_the_documented_prefixes() {
        assert_eq!(SHARED_RUNTIME_NOTICE_PREFIX, "shared runtime ");
        assert_eq!(
            ACTIVE_STREAM_ISOLATION_NOTICE_FRAGMENT,
            "app-server connection while a turn stream was active"
        );
    }
    ...
    fn parses_approval_review_notification_for_current_turn() {
        ...
        assert!(matches!(handling, TurnNotificationHandling::Consumed));
        assert_eq!(
            event_rx.recv().expect("event should be sent"),
    ...
    #[test]
    fn unknown_approval_review_statuses_are_preserved() {
        let notification = AppServerNotification::from_value(json!({
    ...
    fn changed_planning_file_paths_filter_and_canonicalize_file_changes() {
        let paths = changed_planning_file_paths(&json!({
            "type": "fileChange",
            "changes": [
                {
                    "path": "src/main.rs",
    ...
    #[test]
    fn notification_warning_text_preserves_out_of_band_details() {
        let notification = AppServerNotification::from_value(json!({
