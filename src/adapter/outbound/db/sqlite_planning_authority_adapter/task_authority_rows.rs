/*
학습 주석:
이 모듈은 domain의 `TaskAuthorityDocument`와 `PriorityQueueProjection`을 SQLite 테이블 행으로
펼쳐 저장하는 adapter 내부 매핑 계층입니다.

task authority는 원래 하나의 문서처럼 다룰 수 있는 데이터입니다. 하지만 TUI와 app-server 흐름에서는
작업 목록, 의존 관계, queue rank, skipped 이유를 따로 조회하거나 projection해야 합니다. 그래서 이
파일은 문서형 domain 값을 다음 테이블들로 나눕니다.

- `planning_tasks`: task의 현재 정의와 우선순위 계산 결과를 task_id 단위로 저장합니다.
- `planning_task_edges`: depends_on / blocked_by 같은 task 간 관계를 edge 행으로 저장합니다.
- `planning_queue_projection`: active / proposed / skipped queue에 보이는 현재 정렬 결과를 저장합니다.
- `authority_metadata`: task ledger 버전처럼 전체 문서에 붙는 metadata를 저장합니다.

중요한 점은 이 파일이 domain 규칙을 새로 계산하지 않는다는 것입니다. 우선순위나 skipped 여부는 이미
domain/application 쪽에서 계산된 projection으로 들어오며, 이 모듈은 그 결과를 SQLite 조회에 맞게
정규화하고 transaction 안에서 원자적으로 반영합니다.
*/
...
/*
학습 주석:
task authority 관련 테이블을 새 snapshot으로 교체합니다.

이 함수는 store 계층에서 이미 열린 transaction을 받아 실행됩니다. 그래서 이 안의 여러 DELETE/INSERT가
하나의 commit 단위로 묶입니다. 목표는 "task authority 문서와 queue projection이 서로 같은 시점의
상태를 나타내게 하는 것"입니다. task 정의만 바뀌고 queue projection이 예전 상태로 남으면 TUI가
상충되는 정보를 보여줄 수 있으므로, 두 데이터를 한 함수에서 같이 갱신합니다.

갱신 순서도 의도적입니다.
1. queue projection은 전체 snapshot 성격이므로 먼저 비웁니다.
2. task 본문과 edge는 stale 행 삭제 후 upsert합니다.
3. active/proposed/skipped projection을 다시 삽입합니다.
4. ledger version metadata를 갱신해 DB가 어떤 task authority 버전을 반영하는지 남깁니다.
*/
...
/*
학습 주석:
`TaskAuthorityDocument.tasks` 목록을 `planning_tasks`와 `planning_task_edges`에 동기화합니다.

이 함수가 단순히 모든 task를 삭제하고 다시 넣지 않는 이유는 `planning_tasks`가 task_id 기준 upsert
구조이기 때문입니다. 먼저 원하는 task_id 집합을 만들고, 문서에서 사라진 stale task만 골라 삭제합니다.
그 뒤 현재 문서에 남아 있는 task는 순서(`task_order`)와 JSON 원문까지 upsert합니다.

edge는 task 본문과 다르게 매번 해당 task_id의 관계 행을 지우고 다시 삽입합니다. depends_on이나
blocked_by 목록은 순서가 있는 배열이고, 개별 edge의 변경을 diff하는 이득이 작습니다. 현재 task의
관계를 "문서에 적힌 그대로" 다시 쓰는 편이 더 명확합니다.
*/
...
/*
학습 주석:
현재 문서에 더 이상 존재하지 않는 task 행과 그 edge를 삭제합니다.

`desired_task_ids`는 domain 문서에서 온 task_id 집합이고, DB의 `planning_tasks`는 이전 snapshot의
잔여 행을 포함할 수 있습니다. 예를 들어 사용자가 task를 완료/삭제해서 authority 문서에서 빠졌다면
그 task는 DB 조회 결과에서도 사라져야 합니다. 이 helper는 DB에만 남아 있는 stale task를 찾아
edge부터 지운 뒤 task 본문을 지웁니다.

edge를 먼저 지우는 순서는 관계형 저장소에서 흔한 부모-자식 정리 순서입니다. schema의 제약 조건이
바뀌더라도 dangling edge가 남지 않도록 명시적으로 자식 관계를 먼저 제거합니다.
*/
fn delete_stale_task_rows(
    ...
    /*
    학습 주석:
    `statement`가 살아 있으면 rusqlite borrow가 transaction 사용을 계속 붙잡을 수 있습니다.
...
/*
학습 주석:
domain의 `TaskDefinition` 하나를 `planning_tasks` 행 하나로 저장합니다.

테이블에는 조회와 정렬에 자주 쓰는 필드를 column으로 풀어 넣고, 동시에 `content_json`에는 task 전체를
직렬화해서 보존합니다. 이 이중 저장은 의도된 tradeoff입니다. SQL에서는 title/status/priority 같은
필드를 직접 필터링하고 정렬할 수 있고, 필요하면 JSON 원문으로 domain 객체 전체를 복원할 수 있습니다.

`task_order`는 문서 안의 배열 순서를 보존합니다. `combined_priority`는 DB에서 매번 다시 계산하지
않도록 domain method 결과를 snapshot으로 저장합니다. `ON CONFLICT(task_id)`는 같은 task가 다음
snapshot에서 수정되었을 때 같은 row를 갱신하게 합니다.
*/
fn upsert_task_row(
    transaction: &rusqlite::Transaction<'_>,
    index: usize,
...
/*
학습 주석:
task의 관계 배열을 `planning_task_edges` 행들로 저장합니다.

`edge_kind`는 현재 `"depends_on"` 또는 `"blocked_by"`로 들어옵니다. 같은 테이블에 두 종류를 함께
넣으면 task 관계 조회 로직이 한 테이블만 보면 되고, edge_kind column으로 의미를 구분할 수 있습니다.
`edge_order`는 domain 문서의 배열 순서를 보존합니다. 순서 자체가 알고리즘에 필요하지 않더라도,
사람이 보는 출력과 round-trip 복원에서 원래 문서의 의도를 덜 잃게 됩니다.
*/
...
/*
학습 주석:
active/proposed queue에 보이는 task projection을 저장합니다.

여기서 저장하는 값은 task 정의 자체가 아니라 "현재 queue에서 이 task가 어떤 rank로 보이는가"입니다.
그래서 task 본문 테이블과 별도로 `planning_queue_projection`에 들어갑니다. bucket은 caller가
`"active"` 또는 `"proposed"`를 넘기며, item_kind는 일반 task row임을 나타내기 위해 `'task'`로
고정합니다.

`content_json`에는 `PriorityQueueTask` 전체를 저장합니다. projection이 나중에 필드를 더 갖게 되더라도
DB schema를 바로 확장하지 않고 JSON 원문에서 정보를 회수할 수 있게 하기 위한 보존 장치입니다.
*/
...
/*
학습 주석:
queue 계산에서 제외된 task들을 skipped bucket으로 저장합니다.

skipped task는 active/proposed task와 다르게 projection 안에 이미 rank가 있는 구조가 아니라, skipped
목록의 순서 자체가 표시 순서입니다. 그래서 enumerate index에 1을 더해 rank로 저장합니다. item_kind를
`'skipped'`로 넣어 같은 table 안에서도 일반 queue task와 구분합니다.
*/
...
/*
학습 주석:
task authority 관련 데이터와 ledger version metadata를 모두 비웁니다.

이 함수는 "현재 DB가 task authority snapshot을 갖고 있다"는 상태 자체를 제거할 때 쓰는 공개 helper입니다.
row 데이터만 지우고 metadata가 남으면 상위 로직이 DB에 아직 특정 ledger version이 반영되어 있다고
오해할 수 있습니다. 그래서 `clear_task_authority_rows` 다음에 `authority_metadata`의 version key도
명시적으로 삭제합니다.
*/
...
/*
학습 주석:
task authority의 행 데이터만 비웁니다.

삭제 순서는 projection -> edge -> task입니다. queue projection은 task row를 참조하는 조회용 snapshot이고,
edge는 task 사이의 관계 행이며, planning_tasks는 중심 본문입니다. 중심 본문을 먼저 지우면 관계 행이나
projection이 잠깐이라도 고아 데이터처럼 남을 수 있으므로 주변 projection/edge를 먼저 비웁니다.
*/
...
/*
학습 주석:
queue projection snapshot만 삭제합니다.

`replace_task_authority_tables`는 task 본문을 upsert하기 전에 projection을 항상 비웁니다. projection은
rank가 있는 현재 계산 결과라서 부분 upsert보다 전체 재생성이 더 안전합니다. stale projection row가
하나라도 남으면 TUI가 이미 사라진 task를 queue에 보여줄 수 있습니다.
*/
