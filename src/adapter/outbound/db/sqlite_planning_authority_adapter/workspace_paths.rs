...
impl SqlitePlanningAuthorityAdapter {
    ...
    */
    ...
    - `canonical_repo_root`: Git repository 전체를 대표하는 root다. linked worktree라면 worktree
      디렉터리가 아니라 main repository root로 보정된다.
...
/*
workspace가 속한 canonical Git repository root를 찾고, 결과를 process-local cache에 저장한다.

이 함수가 cache를 쓰는 이유는 planning workspace 동작 중 같은 workspace에 대해 root 해석을 여러
번 반복하기 때문이다. Git 명령 실행은 파일 경로 계산보다 훨씬 비싸므로, canonicalized workspace
경로 문자열을 key로 삼아 한 번 계산한 root를 재사용한다. cache 값은 `PathBuf`로 clone해서 반환하므로
caller가 마음대로 소유해도 전역 cache는 안전하게 남는다.
*/
...
/*
canonical repo root cache의 전역 저장소다.

`OnceLock`은 최초 접근 때 한 번만 `Mutex<BTreeMap<...>>`를 만들고 이후에는 같은 객체를 재사용한다.
`Mutex`를 둔 이유는 테스트나 future runtime에서 여러 thread가 동시에 workspace path를 해석해도
cache map 내부가 깨지지 않도록 하기 위해서다.
*/
