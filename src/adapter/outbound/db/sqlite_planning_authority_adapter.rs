...
순서를 항상 지킨다.
1. 새 authority DB의 parent directory를 만든다.
2. SQLite connection을 연다.
3. 기존 DB라면 schema version이 이 binary가 이해할 수 있는 범위인지 검사한다.
4. foreign key enforcement를 켠다.
5. 현재 schema가 필요로 하는 table/index를 보장한다.
