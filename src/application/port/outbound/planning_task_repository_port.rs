...
/*
 * task authority snapshot은 planning task 저장소의 읽기 모델이다.
 * `TaskAuthorityDocument`는 원본 authority 문서이고 `PriorityQueueProjection`은 실행/선택 흐름에서
 * 바로 쓰기 쉬운 큐 관점 투영이다. 둘을 같은 revision으로 묶어야 문서와 큐가 서로 어긋나지 않는다.
 */
pub struct PlanningTaskAuthoritySnapshot {
    // optimistic commit에서 기준점으로 삼는 단조 증가 revision이다.
    pub planning_revision: i64,
    // task authority의 canonical 문서이다. task 상태, 메타데이터, 정책 판단의 원천이다.
    pub task_authority: TaskAuthorityDocument,
    // scheduler/runtime이 우선순위를 계산할 때 다시 파싱하지 않도록 함께 저장한 투영이다.
    pub queue_projection: PriorityQueueProjection,
}

#[derive(Debug, Clone, PartialEq, Eq)]
/*
 * direction authority snapshot은 작업 지시 카탈로그의 저장소 읽기 모델이다.
 * task authority와 같은 revision 체계를 쓰기 때문에 bootstrap, repair, authoring 서비스가
 * "내가 읽은 지시 문서를 기준으로 아직 최신인가"를 같은 방식으로 확인할 수 있다.
 */
pub struct PlanningDirectionAuthoritySnapshot {
    // direction catalog 저장 흐름의 optimistic 기준 revision이다.
    pub planning_revision: i64,
    // 실제 지시 카탈로그 문서이다. authoring/doctor 흐름이 이 내용을 기준으로 draft를 만든다.
    pub directions: DirectionCatalogDocument,
}

#[derive(Debug, Clone, Copy)]
/*
 * task authority commit 요청은 소유권을 가져오지 않고 문서 참조만 빌린다.
 * application service가 이미 만든 문서/큐 투영을 저장소 경계에 넘기는 얇은 명령 객체이며,
 * `observed_planning_revision`으로 lost update를 막는 비교 후 저장 계약을 표현한다.
 */
pub struct PlanningTaskAuthorityCommit<'a> {
    // 호출자가 마지막으로 본 revision이다. `None`이면 현재값과 무관하게 새 snapshot을 쓴다.
    pub observed_planning_revision: Option<i64>,
    // 저장할 task authority 문서 참조이다. 포트 구현체는 필요할 때 복제하거나 직렬화한다.
...
#[derive(Debug, Clone, Copy)]
/*
 * direction authority commit은 task commit과 같은 revision 충돌 모델을 공유한다.
 * 별도 타입으로 분리해 task 문서와 direction 카탈로그가 섞여 저장되는 실수를 타입 수준에서 막는다.
 */
pub struct PlanningDirectionAuthorityCommit<'a> {
    // 호출자가 읽었던 direction authority revision이다.
    pub observed_planning_revision: Option<i64>,
 ...
 * merge, retry, 사용자 안내 같은 후속 정책을 선택할 수 있다.
 */
pub enum PlanningTaskAuthorityCommitResult {
    ...
    Conflict {
        ...
        // 저장소에 이미 존재하는 최신 revision이다.
        current_planning_revision: i64,
    },
}

/*
 * `PlanningTaskRepositoryPort`는 planning authority 문서를 저장하는 outbound 경계이다.
 * application 계층은 파일, SQLite, 테스트 double 중 무엇이 뒤에 있는지 모르고 snapshot 단위로 읽고 쓴다.
 * 메서드는 direction authority와 task authority를 같은 패턴으로 나눠, 서로 다른 문서의 생명주기를
 * 독립적으로 관리하면서도 revision 충돌 처리 방식은 동일하게 유지한다.
 */
pub trait PlanningTaskRepositoryPort: Send + Sync {
    // workspace에 저장된 direction authority snapshot을 읽는다. 아직 없으면 `Ok(None)`이다.
    fn load_direction_authority_snapshot(
        &self,
        // snapshot을 namespace로 분리하는 workspace 기준 경로/식별자이다.
        workspace_dir: &str,
    ) -> Result<Option<PlanningDirectionAuthoritySnapshot>>;

    // direction authority snapshot을 revision 조건부로 저장한다.
    fn commit_direction_authority_snapshot(
        &self,
        // 저장 대상 workspace namespace이다.
        workspace_dir: &str,
        // 저장할 direction catalog와 관측 revision을 담은 명령이다.
        commit: PlanningDirectionAuthorityCommit<'_>,
    ) -> Result<PlanningTaskAuthorityCommitResult>;

    // workspace의 direction authority snapshot을 제거해 bootstrap/seed가 다시 만들 수 있게 한다.
    fn clear_direction_authority_snapshot(&self, workspace_dir: &str) -> Result<()>;

    // workspace에 저장된 task authority와 큐 투영을 같은 revision으로 읽는다.
    fn load_task_authority_snapshot(
        &self,
        // snapshot을 찾을 workspace namespace이다.
        workspace_dir: &str,
    ) -> Result<Option<PlanningTaskAuthoritySnapshot>>;

    // task authority와 queue projection을 한 revision으로 조건부 저장한다.
...
impl PlanningTaskRepositoryPort for NoopPlanningTaskRepositoryPort {
    ...
    fn load_direction_authority_snapshot(
        &self,
        // map key로 쓰이는 workspace namespace이다.
        workspace_dir: &str,
    ) -> Result<Option<PlanningDirectionAuthoritySnapshot>> {
        // Mutex를 잠근 동안만 map을 보고, 밖으로는 owned snapshot을 돌려 잠금 수명을 짧게 유지한다.
    ...
    // direction snapshot을 optimistic revision 규칙으로 갱신한다.
    fn commit_direction_authority_snapshot(
        &self,
        // 저장할 workspace namespace이다.
        workspace_dir: &str,
        // 저장할 문서 참조와 호출자가 관측한 revision이다.
        commit: PlanningDirectionAuthorityCommit<'_>,
    ) -> Result<PlanningTaskAuthorityCommitResult> {
        ...
        store.insert(
            ...
            PlanningDirectionAuthoritySnapshot {
                planning_revision,
                // commit 객체는 참조만 들고 있으므로 in-memory store에는 owned document로 복제한다.
    ...
    // task authority snapshot 전역 map에서 workspace별 값을 복제해 반환한다.
    fn load_task_authority_snapshot(
        &self,
        // map key로 쓰이는 workspace namespace이다.
        workspace_dir: &str,
    ) -> Result<Option<PlanningTaskAuthoritySnapshot>> {
        // snapshot은 clone되어 반환되므로 caller가 값을 수정해도 store 내부 상태는 바뀌지 않는다.
    ...
    // task authority 문서와 큐 투영을 같은 revision으로 저장한다.
    fn commit_task_authority_snapshot(
        &self,
        // 저장할 workspace namespace이다.
        workspace_dir: &str,
        // 저장할 task authority, queue projection, observed revision이다.
        commit: PlanningTaskAuthorityCommit<'_>,
    ) -> Result<PlanningTaskAuthorityCommitResult> {
        ...
        store.insert(
            ...
            PlanningTaskAuthoritySnapshot {
                planning_revision,
                // 참조로 받은 task authority를 store가 소유하도록 복제한다.
                task_authority: commit.task_authority.clone(),
                // task authority와 동시에 산출된 큐 투영도 같은 snapshot에 넣는다.
...
/*
 * task authority용 전역 in-memory store이다.
...
/*
 * direction authority용 전역 in-memory store이다.
