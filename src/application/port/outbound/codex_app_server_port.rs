...
/*
 * CodexAppServerPort is the legacy app-server shaped boundary. Newer application
 * services depend on narrower capability ports, but the concrete app-server
 * adapter still implements this single surface. The blanket impls below split it
 * back into startup probing, session catalog, and interactive turn runtime
 * capabilities without forcing the adapter graph to hold three wrappers.
 */
pub trait CodexAppServerPort: Send + Sync {
    // Startup checks are read before the TUI owns the terminal event loop.
    fn load_startup_context(&self) -> Result<AppServerStartupContext>;

    // Recent sessions are app-server backed but independent from long turn streams.
    fn load_recent_sessions(&self, limit: usize) -> Result<SessionCatalog>;

    /*
     * App-server supports manual handoff but not direct local interruption.
     * Keeping this default here makes fakes and adapters report the same control
     * semantics unless a future runtime provides a different truth source.
     */
    fn runtime_control_truth(&self) -> ConversationRuntimeControlTruth {
        ConversationRuntimeControlTruth::codex_app_server()
    }

    // Snapshot reads rehydrate an existing thread before the TUI starts a turn.
    fn load_conversation_snapshot(&self, thread_id: &str) -> Result<ConversationSnapshot>;

    // Stop requests are coarse app-server control, not per-turn stream messages.
    fn request_stop_all_sessions(&self) -> Result<()>;

    /*
     * A new-thread stream owns both thread creation and first prompt execution.
     * The result only means the worker launched; transcript, planning-file changes,
     * and terminal attachment observations arrive on event_sender.
     */
    ...
    /*
     * Existing-thread turns reuse a known thread id. The stream contract is the
     * same channel of ConversationStreamEvent values, but attachment semantics are
     * reattach rather than first launch.
     */
    fn run_turn_stream(
        &self,
        thread_id: &str,
...
/*
 * StartupService depends on StartupProbePort. This blanket impl lets the legacy
 * adapter satisfy that focused dependency while the app entrypoints still share a
 * single Arc<CodexAppServerAdapter>.
 */
...
/*
 * SessionService asks for a SessionCatalogRequest that already includes
 * workspace context. The legacy app-server call only accepts a limit today, so
 * the bridge intentionally drops the extra request fields at this compatibility
 * boundary.
 */
...
/*
 * ConversationService depends on InteractiveTurnRuntimePort. Delegating every
 * method here keeps the stream worker path independent from app-server process
 * setup while preserving the concrete adapter's existing implementation.
 */
...
mod tests {
    ...
    /*
     * The fake implements only the legacy port and records the arguments that the
     * narrower capability traits should forward. That makes these tests about the
     * compatibility layer, not about app-server JSON-RPC behavior.
     */
    #[derive(Default)]
    ...
    impl CodexAppServerPort for FakeCodexAppServerPort {
        fn load_startup_context(&self) -> anyhow::Result<AppServerStartupContext> {
            /*
             * Startup attachment uses the launch profile because this path models
             * a fresh app-server connection established during shell boot.
             */
        ...
        fn load_conversation_snapshot(
            ...
            // Snapshot identity must pass through unchanged for resumed threads.
        ...
        fn run_new_thread_stream(
            ...
            // Record launch arguments before emitting synthetic stream events.
            ...
            /*
             * A real new-thread stream first tells the TUI that app-server launch
             * attached a terminal bridge, then eventually sends a terminal event.
             */
            let _ =
        ...
        fn run_turn_stream(
            ...
            // Existing-thread streams must retain both the thread id and prompt.
            self.turn_streams
                .lock()
            ...
            /*
             * Reattached streams use a different attachment profile from first
             * launch; the TUI footer relies on that distinction when reporting
    ...
    fn session_catalog_port_delegates_to_the_legacy_codex_port() {
        ...
        /*
         * The compatibility bridge intentionally maps SessionCatalogRequest to
         * the legacy limit-only call. If workspace-aware filtering is added to
         * app-server later, this test is the prompt to update the bridge.
         */
    ...
    fn interactive_turn_runtime_port_delegates_snapshot_and_stream_calls() {
        ...
        /*
         * Attachment event ordering is part of the stream contract. Conversation
         * runtime observes it before the terminal TurnCompleted event, so the
         * reducer can surface launch/reattach state without guessing from thread
         * creation mode.
         */
        assert_eq!(
            new_thread_receiver
