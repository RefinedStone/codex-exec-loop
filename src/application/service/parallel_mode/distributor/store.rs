/*
 * 학습 주석: distributor store는 parallel mode delivery queue의 persistence adapter다. planning
...
/*
학습 주석: distributor queue root는 pool root 아래의 durable queue mirror입니다. 실제 source of
truth는 planning authority의 runtime queue projection이지만, `.distributor-queue/<id>.json` 파일은
운영자가 queue item을 확인하고 테스트가 store-backed recovery를 검증하는 데 쓰입니다.
*/
fn distributor_queue_root(pool_root: &Path) -> PathBuf {
    pool_root.join(".distributor-queue")
}

// 학습 주석: queue record path 계산을 한 곳에 두어 writer와 test loader가 같은 mirror layout을 공유한다.
...
/*
학습 주석: queue item id는 slot, agent, enqueue timestamp를 합친 runtime record key입니다.
timestamp까지 포함해 같은 agent/slot이 여러 번 결과를 내더라도 queue record 파일명이 겹치지
않습니다. filesystem path로도 쓰이므로 마지막에 sanitize를 거칩니다.
*/
...
/*
학습 주석: queue order key는 RFC3339 timestamp에서 숫자만 뽑아 만든 정렬 키입니다. 문자열
timestamp 정렬도 대체로 가능하지만, u64 key를 함께 저장하면 persistence layer나 UI projection이
명시적인 queue ordering 값을 사용할 수 있습니다.
*/
pub(super) fn queue_order_key_from_timestamp(timestamp: &str) -> u64 {
    // 학습 주석: timezone separator와 punctuation을 제거한 숫자 prefix만 사용해 JSON/DB projection 모두에서 정렬한다.
    timestamp
        .chars()
...
#[cfg(test)]
/*
학습 주석: 테스트용 loader는 파일 미러에서 queue record를 다시 읽어 restart/recovery 시나리오를
검증합니다. production flow는 planning authority projection을 우선하지만, 파일 mirror가 깨지지
않고 order를 보존하는지 확인하려면 이 경로가 필요합니다.
*/
pub(crate) fn load_distributor_queue_records(
    ...
    let Ok(entries) = fs::read_dir(queue_root) else {
        // 학습 주석: mirror directory가 없다는 것은 아직 distributor queue가 생성되지 않았다는 정상 상태다.
        return Vec::new();
    };

    /*
     * 학습 주석: test loader는 corrupt entry를 hard fail하지 않고 건너뛴다. production source는
    ...
    let mut records = entries
        ...
        .collect::<Vec<_>>();
    // 학습 주석: enqueue timestamp를 primary key로, queue item id를 tie breaker로 써 restart 후 display order를 안정화한다.
...
/*
학습 주석: queue record 저장은 planning authority와 filesystem mirror를 모두 갱신합니다.
먼저 authority projection을 upsert해 application이 읽는 실시간 상태를 갱신하고, 그 다음 JSON
파일을 temp file + rename으로 씁니다. 이 순서와 atomic-ish rename은 프로세스 중단 시 부분
JSON이 최종 파일명으로 남는 위험을 줄입니다.
*/
pub(super) fn write_distributor_queue_record(
    ...
    // 학습 주석: 먼저 authority projection을 갱신해야 supervisor snapshot과 distributor processor가 최신 queue를 본다.
    planning_authority
    ...
    // 학습 주석: temp file write가 성공한 뒤 rename해야 partially written JSON이 canonical path에 남지 않는다.
...
/*
학습 주석: block_distributor_queue_record는 delivery의 공통 실패 전이입니다. queue state와
integration state를 blocked로 바꾸고, 최초 recovery_note를 보존하며, 최신 integration_note에는
현재 실패 원인을 기록합니다. lease가 있으면 session detail도 failed로 갱신해 supervisor detail과
completion feed가 queue block을 agent session 관점에서도 보여 줍니다.

반환 문자열은 caller가 TUI notice로 바로 표시할 수 있는 한 줄 요약입니다.
*/
pub(super) fn block_distributor_queue_record(
    ...
    // 학습 주석: 최초 recovery note는 root cause를 보존하고, integration note는 가장 최근 실패를 계속 갱신한다.
    ...
    // 학습 주석: lease가 있으면 queue item뿐 아니라 session detail history도 failed 상태로 투영한다.
