...
pub(super) fn distributor_push_source_branch(
    ...
    // 학습 주석: capability snapshot을 record에 보관해 blocked supervisor 화면이 "왜 push 불가인지" 즉시 설명하게 합니다.
    ...
    if !capabilities.push_ready() {
        // 학습 주석: push가 준비되지 않은 경우는 일시 장애일 수 있으므로 queue record를 blocked retry 지점으로 남깁니다.
    ...
    // 학습 주석: session detail은 queue record보다 operator-facing timeline에 가깝기 때문에 실패해도 delivery를 중단하지 않습니다.
    ...
    if let Err(error) = github_automation.push_branch(&repo_root, &record.branch_name, false) {
        // 학습 주석: 실제 push 실패는 remote/auth/network 상태와 연결되므로 같은 block path로 복구 가능하게 만듭니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
...
pub(super) fn distributor_ensure_pull_request(
    ...
    // 학습 주석: PR 조작은 push와 다른 capability라 여기서 gh binary/auth 상태를 다시 읽고 기록합니다.
    let repo_root = resolution.context.repo_root.clone();
    let capabilities = github_automation.inspect_capabilities(&repo_root);
    record.github_capabilities = Some(capabilities.clone());
    if !capabilities.github_ready() {
        // 학습 주석: binary 부재와 auth 부재 중 더 직접적인 원인을 골라 block note를 짧고 실행 가능하게 만듭니다.
    ...
    // 학습 주석: PR pending detail은 "remote branch는 있음, GitHub PR 표면을 만드는 중"인 중간 상태를 노출합니다.
    ...
    // 학습 주석: ensure는 idempotent boundary입니다. retry 시 기존 PR을 재사용해야 queue가 중복 PR을 만들지 않습니다.
...
pub(super) fn distributor_check_pull_request_merge_readiness(
    ...
    let Some(pr_number) = record.pull_request_number else {
        // 학습 주석: PR 번호가 없다면 이전 ensure 단계의 durable write가 깨진 것이므로 operator recovery로 넘깁니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
    ...
    // 학습 주석: merge pending detail은 이후 local integration/cherry-pick 단계로 넘어가기 전의 원격 검증 상태입니다.
    ...
    let repo_root = resolution.context.repo_root.clone();
    // 학습 주석: readiness는 queue record의 저장 값이 아니라 GitHub 현재 상태를 다시 읽어 drift를 잡습니다.
    ...
    if !pull_request.state.eq_ignore_ascii_case("open") {
        // 학습 주석: closed/merged PR은 source branch와 queue state가 이미 외부에서 변했을 수 있어 자동 통합을 멈춥니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
    ...
    if pull_request.is_draft {
        // 학습 주석: draft PR은 사람이 아직 통합 표면을 확정하지 않은 신호라 distributor가 로컬 반영하지 않습니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
    ...
    if pull_request.base_branch != DISTRIBUTOR_INTEGRATION_BRANCH {
        // 학습 주석: base drift는 다른 integration lane으로 향한 PR일 수 있으므로 현재 distributor queue에서 통합하지 않습니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
    ...
    if pull_request.head_branch != record.branch_name {
        // 학습 주석: head drift는 queue record가 가리키는 agent result와 PR content가 달라졌다는 강한 불일치입니다.
        return block_distributor_queue_record(
            planning_authority,
            &resolution.context.repo_root,
            &resolution.context.pool_root,
