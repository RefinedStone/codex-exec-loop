...
pub(super) fn ensure_pool_baseline_branch(
    ...
    if reset_to_current_head && let Some(head_sha) = resolve_branch_head(repo_root, "HEAD") {
        ...
        사용되어야 합니다.
        */
        // 학습 주석: existed flag는 "새 baseline을 만들었는가"와 "기존 baseline을 이동했는가"를 summary에서 구분하게 합니다.
    ...
    */
    // 학습 주석: remote ref를 명시 경로로 확인해 로컬 branch 이름과 remote tracking branch 이름을 혼동하지 않습니다.
...
pub(super) fn provision_missing_slots(
    ...
    */
    ...
    for slot_number in 1..=DEFAULT_POOL_SIZE {
        ...
        // 학습 주석: git command boundary는 Path를 문자열로 넘겨야 하므로 여기서만 display string으로 변환합니다.
...
pub(super) fn reset_reusable_detached_baseline_slots(
    ...
    절대 reset하지 않습니다.
    */
    // 학습 주석: baseline head를 못 찾으면 reset 기준이 없으므로 모든 slot을 관찰 전용으로 둡니다.
    ...
    for slot_number in 1..=DEFAULT_POOL_SIZE {
        ...
        let Some(worktree_record) = worktree_records
            ...
            // 학습 주석: inventory에 없는 slot은 provisioning/inspection 단계가 다루며, reset 대상이 아닙니다.
            continue;
        };
        ...
        // 학습 주석: head SHA와 worktree dirtiness를 함께 봐야 stale baseline과 dirty idle slot을 모두 잡을 수 있습니다.
