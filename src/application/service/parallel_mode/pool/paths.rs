...
pub(in crate::application::service::parallel_mode) fn derive_default_pool_root(
    ...
    // 학습 주석: repository root가 filesystem root처럼 이름을 얻을 수 없는 경로여도 pool
    // 계산은 계속되어야 하므로 표시 이름만 안전한 fallback으로 바꿉니다.
    let repo_name = canonical_repo_root
        .file_name()
        .and_then(|name| name.to_str())
        .filter(|name| !name.is_empty())
        .unwrap_or("workspace");
    // 학습 주석: sibling directory를 기준으로 잡아 source checkout 내부에 slot worktree가
    // 생기지 않게 합니다. parent가 없으면 root 자체를 기준으로 삼아 path 계산만 유지합니다.
...
fn stable_short_hash(value: &str) -> String {
    ...
    // 학습 주석: path key만 만들 목적이므로 별도 dependency 없이 고정된 byte 순회로
    // 플랫폼마다 같은 checkout 경로가 같은 짧은 suffix를 얻도록 합니다.
...
pub(super) fn parse_worktree_records(output: &str) -> Vec<GitWorktreeRecord> {
    // 학습 주석: porcelain record는 줄 순서가 고정되어 있지만 중간 필드가 빠질 수 있습니다.
    // local builder가 record 하나의 부분 상태만 들고 있다가 path가 있는 묶음만 최종 inventory로 냅니다.
    #[derive(Default)]
    struct Builder {
        path: Option<PathBuf>,
    ...
    for line in output.lines().chain(std::iter::once("")) {
        ...
        // 학습 주석: `refs/heads/` prefix를 제거해 board와 reconciler가 local branch 이름만
        // 비교하게 합니다. detached record는 branch line 없이 별도 marker로 들어옵니다.
...
pub(in crate::application::service::parallel_mode) fn inspect_slot_git_status(
    ...
    for line in status_output.lines().skip(1) {
        // 학습 주석: porcelain v1에서 `??`는 index/worktree column 의미가 없는 untracked
        // marker라 staged/unstaged 판정으로 흘리지 않고 별도 flag만 세웁니다.
        if line.starts_with("??") {
            status.has_untracked = true;
            continue;
        }

        // 학습 주석: porcelain v1의 첫 두 칸은 각각 index(X)와 worktree(Y) 상태입니다.
        // pool cleanup은 두 종류의 사용자 변경을 따로 보고해야 자동 reset 판단을 보수적으로 할 수 있습니다.
    ...
    // 학습 주석: status output에는 merge/rebase 진행 중 metadata가 항상 직접 드러나지 않으므로
    // git dir을 별도로 찾아 자동 조작을 막아야 하는 pending 상태까지 합산합니다.
