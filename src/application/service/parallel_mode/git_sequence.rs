/*
 * 학습 주석: git_sequence는 parallel mode가 수행하는 destructive-ish git 작업을 작은 단계와
...
pub(super) struct GitCommandStep {
    // 학습 주석: label은 operator notice와 failure summary에 노출되는 사람이 읽는 단계 이름이다.
    label: String,
    // 학습 주석: args는 `git` 뒤에 붙는 argv 그대로라 quoting 없이 실행/진단을 재현할 수 있다.
    args: Vec<String>,
}

impl GitCommandStep {
    // 학습 주석: 생성자는 caller가 &str 배열을 넘겨도 owned report로 보존되도록 label과 args를 소유 문자열로 바꾼다.
    pub(super) fn new(
...
pub(super) struct GitCommandStepReport {
    // 학습 주석: report에도 label/args를 복사해 caller가 원래 step collection을 보관하지 않아도 진단할 수 있다.
    pub(super) label: String,
    pub(super) args: Vec<String>,
    // 학습 주석: spawn 실패는 exit code가 없으므로 None으로 두어 "git이 실패"와 "git을 실행하지 못함"을 구분한다.
    pub(super) exit_code: Option<i32>,
    // 학습 주석: stdout/stderr는 trimmed string으로 저장해 TUI notice와 test assertion이 불필요한 개행을 다루지 않게 한다.
    pub(super) stdout: String,
    pub(super) stderr: String,
}

impl GitCommandStepReport {
    // 학습 주석: git convention상 exit code 0만 성공으로 보고, signal/launch failure는 실패로 남긴다.
    pub(super) fn succeeded(&self) -> bool {
        self.exit_code == Some(0)
    }

    /*
     * 학습 주석: failure summary는 상위 recovery notice에 붙을 한 줄 진단이다. stderr를 우선하는 이유는
...
pub(super) struct GitCommandSequenceReport {
    // 학습 주석: sequence label은 "slot cleanup"처럼 여러 git step을 묶는 상위 작업 이름이다.
    pub(super) label: String,
    // 학습 주석: steps에는 실행된 단계만 남으므로 실패 후 실행하지 않은 step은 report에 들어가지 않는다.
    pub(super) steps: Vec<GitCommandStepReport>,
}

impl GitCommandSequenceReport {
    // 학습 주석: 전체 sequence 성공은 실행된 모든 step이 성공했는지로 판단한다.
    pub(super) fn succeeded(&self) -> bool {
        self.steps.iter().all(GitCommandStepReport::succeeded)
    }

    // 학습 주석: 상위 caller는 보통 첫 실패 원인만 operator에게 보여 주면 충분하므로 첫 실패 summary만 반환한다.
...
pub(super) fn run_git_sequence(
    ...
    for step in steps {
        ...
        reports.push(report);
        // 학습 주석: 실패 뒤의 reset/clean/cherry-pick 단계를 실행하지 않는 것이 worktree 보존에 더 안전하다.
        if !succeeded {
            break;
        }
    }
...
fn run_git_step(step: GitCommandStep) -> GitCommandStepReport {
    ...
    match output {
        ...
        // 학습 주석: git binary 실행 자체가 실패해도 같은 report shape로 올려 상위 정책이 동일하게 처리한다.
...
mod tests {
    ...
    fn git_sequence_stops_at_first_failed_step_and_keeps_diagnostic() {
        /*
         * 학습 주석: 실제 cleanup/recovery sequence에서 중간 단계가 실패하면 이후 destructive step을
