...
fn seed_stale_running_dispatch_command(
    authority: &SqlitePlanningAuthorityAdapter,
    workspace_dir: &str,
    epoch_id: u64,
) {
    let command = ParallelModeDispatchCommandSnapshot::dispatch_ready_queue(
        ParallelModeAutomationTrigger::TaskIntakeAfterEpoch,
        Some(format!("stale-head-{epoch_id}")),
        Some(epoch_id),
        "2026-05-12T00:00:00Z",
    );
    authority
        .enqueue_runtime_dispatch_command(workspace_dir, &command)
        .expect("stale dispatch command should enqueue");
    let mut running = authority
        .try_claim_next_runtime_dispatch_command(workspace_dir, "owner-stale")
        .expect("stale dispatch command should claim")
        .expect("stale dispatch command should exist");
    running.updated_at = "2020-05-12T00:00:00+00:00".to_string();
    authority
        .update_runtime_dispatch_command(workspace_dir, &running)
        .expect("stale running dispatch command should persist");
}
...
#[test]
fn pending_dispatch_wake_skips_stale_epoch_command_and_surfaces_current_one() {
    let repo = TempGitRepo::new("orchestrator-stale-epoch-wake");
    let workspace_dir = repo.workspace_dir();
    let authority = Arc::new(SqlitePlanningAuthorityAdapter::new());
    let planning = build_test_planning_services(authority.clone());
    bootstrap_planning_workspace(&planning, &workspace_dir);
    ...
    authority
        .enqueue_runtime_dispatch_command(
            ...
            &ParallelModeDispatchCommandSnapshot::dispatch_ready_queue(
                ParallelModeAutomationTrigger::TaskIntakeAfterEpoch,
                Some("current-head-224".to_string()),
                Some(224),
                "2026-05-12T00:00:01Z",
            ),
        )
        .expect("current durable dispatch command should enqueue");

    let wake = service
        .pending_dispatch_wake(&workspace_dir, 224)
        .expect("durable dispatch commands should be inspectable")
        .expect("current durable dispatch command should trigger a wake");
    assert_eq!(
        wake.trigger,
        ParallelModeAutomationTrigger::TaskIntakeAfterEpoch
    );
...
fn pending_dispatch_wake_skips_fresh_running_command_with_matching_lease() {
    ...
    let queue_head = planning_projection
        .queue_head()
        .expect("queue head should exist for lease-backed recovery wake");
    service
        .enqueue_dispatch_commands_for_event(
            &workspace_dir,
            ParallelModeRuntimeEvent::TaskIntakeCommitted,
    ...
    authority
        .upsert_runtime_slot_lease(
            ...
            &ParallelModeSlotLeaseSnapshot::new(
                "slot-1",
                queue_head.task_id.clone(),
                queue_head.task_title.clone(),
                "agent-1",
                "akra-agent/slot-1/test",
                format!("{workspace_dir}/slot-1"),
                crate::domain::parallel_mode::ParallelModeSlotLeaseState::Leased,
                "2026-05-12T00:00:00Z",
                None,
            ),
        )
        .expect("matching slot lease should persist");
...
#[test]
fn dispatch_tick_enqueue_trigger_requeues_after_stale_running_duplicate_command_id() {
    let repo = TempGitRepo::new("orchestrator-stale-running-duplicate-enqueue");
    let workspace_dir = repo.workspace_dir();
    let authority = Arc::new(SqlitePlanningAuthorityAdapter::new());
    let planning = build_test_planning_services(authority.clone());
    bootstrap_planning_workspace(&planning, &workspace_dir);
    ...
    let mut running = authority
        .try_claim_next_runtime_dispatch_command(&workspace_dir, "owner-stale")
        .expect("stale running command should claim")
        .expect("stale running command should exist");
    running.updated_at = "2020-05-12T00:00:00+00:00".to_string();
    authority
        .update_runtime_dispatch_command(&workspace_dir, &running)
...
#[test]
fn dispatch_tick_reclaims_stale_running_dispatch_command_after_restart() {
    let repo = TempGitRepo::new("orchestrator-stale-running-claim");
    let workspace_dir = repo.workspace_dir();
    let authority = Arc::new(SqlitePlanningAuthorityAdapter::new());
    let planning = build_test_planning_services(authority.clone());
    bootstrap_planning_workspace(&planning, &workspace_dir);
...
fn dispatch_tick_does_not_recover_fresh_running_command_with_matching_lease() {
    ...
    let queue_head = planning_projection
        .queue_head()
        .expect("queue head should exist for lease-backed recovery tick");
    service
        .enqueue_dispatch_commands_for_event(
            &workspace_dir,
            ParallelModeRuntimeEvent::TaskIntakeCommitted,
...
#[test]
fn dispatch_tick_skips_stale_epoch_command_and_launches_current_one() {
    let repo = TempGitRepo::new("orchestrator-stale-epoch-claim");
    let workspace_dir = repo.workspace_dir();
    let authority = Arc::new(SqlitePlanningAuthorityAdapter::new());
    let planning = build_test_planning_services(authority.clone());
    bootstrap_planning_workspace(&planning, &workspace_dir);
