/*
 * 학습 주석: session_detail/store.rs는 parallel agent session detail의 persistence adapter다.
...
pub(crate) fn read_agent_session_detail_record(
    ...
    // 학습 주석: mirror parse 실패는 authority-backed state를 계속 쓰게 하기 위해 absence로 접는다.
...
pub(crate) fn agent_session_detail_record_path(pool_root: &Path, session_key: &str) -> PathBuf {
    ...
    // 학습 주석: session_key의 의미 있는 ASCII segment는 보존하고 path separator나 기호는 `_`로 접는다.
