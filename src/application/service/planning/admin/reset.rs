// reset facade는 filesystem/write 작업을 하므로 실패 가능성을 caller에게 전파해야 한다.
// anyhow::Result를 쓰면 lower planning service의 context 있는 error를 admin layer까지 그대로 올릴 수 있다.
use anyhow::Result;

// PlanningResetTarget은 queue, directions, all 중 무엇을 초기화할지 나타내는 domain-facing 선택지이다.
// admin facade는 문자열 form parsing을 이미 끝낸 typed target만 받아 application service로 넘긴다.
...
// facade service는 shared planning dependency bundle을 들고 있고, outcome은 inbound adapter로 돌려줄 DTO이다.
...
impl PlanningAdminFacadeService {
    ...
    pub fn reset_workspace(
        &self,
        // target은 이미 parse/validation을 통과한 typed reset 범위이다. 여기서는 문자열을 다시 해석하지 않는다.
        target: PlanningResetTarget,
    ) -> Result<PlanningAdminResetOutcome> {
        // planning.workspace.reset_workspace가 실제 rewrite/remove를 수행하는 authoritative path이다.
        // facade는 self.workspace_dir을 문자열로 넘겨 현재 admin instance가 관리하는 workspace만 대상으로 삼는다.
        let result = self
        ...
        // 같은 응답에 담는다. admin 화면은 이 snapshot으로 validation panel을 갱신한다.
        let doctor = self
        ...
        // reset service 결과와 doctor projection을 admin DTO에 합칩니다. 이 boundary에서 domain type을
        // 직접 노출하지 않기 때문에 web API와 page renderer가 같은 stable response shape을 사용할 수 있다.
