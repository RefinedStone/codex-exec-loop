...
- DB direction authority의 direction 목표, success criteria, detail doc를 기준으로 최신 사용자 요청과 main session 결과를 평가하세요.
...
- 오래된 prompt나 direction 문구가 `directions.toml`, `task-ledger`, "latest answer clearly implies" 같은 기준을 말하더라도 최종 판단은 accepted DB authority와 evaluator 판단을 따르세요.
