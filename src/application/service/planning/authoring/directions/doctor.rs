/*
Directions doctor는 operator가 직접 고친 direction catalog와 supporting markdown files 사이의 불일치를 복구하는
service flow다. catalog 안의 path mapping을 canonical planning 경로로 고치고, 누락된 detail doc/queue-idle
prompt 파일을 기본 본문으로 생성한 뒤 validation까지 다시 돌려 outcome으로 돌려준다.
*/
...
impl PlanningDirectionsService {
    #[allow(dead_code)]
    // doctor_workspace는 direction authority를 "검사 후 필요한 최소 복구만 적용"하는 entrypoint다.
    // catalog 수정과 file 생성은 staging map에 모았다가 뒤에서 한 번씩 쓰고, 마지막 validation report로 남은 문제를 알려 준다.
    pub fn doctor_workspace(&self, workspace_dir: &str) -> Result<PlanningDoctorOutcome> {
        // complete workspace는 현재 direction catalog와 supporting files를 판단할 기준점이다.
        // directions는 수정 가능한 사본으로 분리해, 실제 변경이 있을 때만 commit하도록 한다.
        let workspace = self.load_complete_workspace(workspace_dir)?;
        let mut directions = workspace.directions.clone();
        // outcome counters는 doctor가 무엇을 실제로 고쳤는지 admin/TUI가 설명할 수 있게 하는 audit 값이다.
        ...
        // supporting file 생성 요청은 path별로 모아 둔다. 같은 target path를 여러 direction이 공유해도
        // insert 결과로 중복 생성을 피하고 counter를 한 번만 올릴 수 있다.
        let mut pending_supporting_files = std::collections::HashMap::<String, String>::new();

        // 각 direction의 detail_doc_path는 `docs/planning/directions/...md` 안쪽의 유효 markdown이어야 한다.
        // 비어 있거나 잘못된 path는 direction id에서 파생한 기본 path로 되돌린다.
        for direction in directions.directions.clone() {
            ...
            if configured_path != Some(target_path.as_str()) {
                // catalog mapping을 고치는 것은 파일 생성과 별개다. direction 문서가 어디에 있어야 하는지
                // authority부터 canonical하게 만든 뒤, 아래에서 파일 존재 여부를 채운다.
            ...
            if self
                ...
                && pending_supporting_files
                    ...
                    .is_none()
            {
                // detail doc이 없으면 direction metadata에서 기본 markdown을 만든다. best-effort load가 None인
                // 경우만 생성해, 이미 사용자가 작성한 파일을 doctor가 덮어쓰지 않게 한다.
                created_detail_doc_files += 1;
            }
        }

        // queue-idle prompt는 policy가 ReviewAndEnqueue이거나 이미 prompt path가 설정된 경우에만 복구한다.
        // 다른 policy에서 빈 path인 workspace에 불필요한 prompt 파일을 새로 만들지 않기 위한 gate다.
        ...
        if should_repair_queue_idle_prompt {
            ...
            if configured_prompt_path != Some(target_prompt_path.as_str()) {
                // prompt mapping도 contract directory 밖이면 기본 queue-idle prompt path로 되돌린다.
                // worker orchestration은 이 path를 통해 idle queue review prompt를 읽는다.
            ...
            if self
                ...
                && pending_supporting_files
                    ...
                    .is_none()
            {
                // queue-idle prompt 파일이 없으면 shared default prompt를 생성한다. 이 default는 worker가 queue idle
                // 상태에서 제안/정리를 요청할 때 쓰는 evaluator contract의 시작점이다.
                created_queue_idle_prompt_file = true;
            }
        }

        if directions != workspace.directions {
            // catalog 사본이 실제로 달라졌을 때만 commit한다. file만 생성한 doctor run은 direction authority
        ...
        for (relative_path, body) in pending_supporting_files {
            // supporting files는 catalog commit 뒤에 쓴다. catalog가 canonical path를 가리키고, 그 path에
            // 기본 body가 존재하는 상태로 workspace를 닫기 위한 순서다.
            self.planning_workspace_port
                .replace_planning_workspace_file(workspace_dir, &relative_path, Some(&body))?;
        }

        // 복구 후 validation을 다시 수행해 caller가 "수정은 했지만 아직 남은 문제"를 바로 볼 수 있게 한다.
    ...
    #[allow(dead_code)]
    // validate_active_workspace는 doctor가 복구한 결과를 planning validator 관점으로 다시 확인한다.
    // 기본 authority seed를 보장한 뒤 direction catalog, result output, supporting file presence를 같은 report에 합친다.
    fn validate_active_workspace(&self, workspace_dir: &str) -> Result<PlanningValidationReport> {
        // doctor는 direction/supporting file repair에 집중하지만 validation에는 result output 같은 기본 authority 파일도 필요하다.
        // seed service가 없으면 빈 workspace validation이 unrelated failure로 끝난다.
        ...
        let mut result =
            self.planning_validation_service
                .validate_workspace_files(PlanningWorkspaceFiles {
                    ...
                    // doctor validation은 task authority 자체를 복구하지 않는다. direction/supporting file 문제를
                    // 보려는 흐름이라 최소 빈 task authority JSON으로 validator 입력을 채운다.
        ...
        if let Some(directions) = result.directions.as_ref() {
            // 첫 validation pass가 direction catalog를 파싱했을 때만 supporting file existence를 검사한다.
            // path lookup은 workspace port의 best-effort load로 감싸 파일 본문이 아니라 존재 여부만 validator에 넘긴다.
            self.planning_validation_service
