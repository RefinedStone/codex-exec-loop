...
/*
 * The planning task tool is the LLM-facing write API for task authority.  It is
 * deliberately narrower than the full planning admin surface: workers can read
 * tasks, then apply one create/update through PlanningTaskMutationService, but
 * they cannot edit files, rewrite SQL, or submit broad backlog batches.
 */
...
pub enum PlanningTaskToolRequest {
    // The tagged enum mirrors the JSON `op` field so invalid or misspelled
    // operations fail before they reach mutation code.
...
pub struct PlanningTaskToolListRequest {
    pub version: u32,
    // Empty status means "show all tasks"; explicit statuses let the model
    // narrow to ready/proposed/blocked without inventing its own filters.
    #[serde(default)]
...
pub struct PlanningTaskToolCreateRequest {
    pub version: u32,
    // Mutations require an explicit apply flag so model prompts can ask for a
    // dry planning step before allowing authority changes.
    pub apply: bool,
    pub source_turn_id: Option<String>,
    // Flattening keeps requests easy for models to produce:
    // {"op":"create_task","title":"..."} instead of nested task objects.
    #[serde(flatten)]
...
pub struct PlanningTaskCreatePayload {
    // This payload intentionally matches PlanningTaskCreateInput except for its
    // JSON ergonomics, so conversion stays mechanical and validation remains in
...
pub struct PlanningTaskUpdatePayload {
    // Update fields are optional patches.  Missing values preserve the current
    // task, while Some(Vec::new()) intentionally clears dependencies.
    pub task_id: String,
    pub direction_id: Option<String>,
    pub direction_relation_note: Option<String>,
...
pub struct PlanningTaskToolResponse {
    ...
    // Mutation metadata is surfaced directly so workers can avoid double
    // applying final planning_task_commands after a successful tool call.
...
pub struct PlanningTaskToolService {
    ...
    // Writes go through the same service used by the app runtime, keeping tool
    // calls on the revision/validation/audit path instead of creating a second
...
impl PlanningTaskToolService {
    ...
    pub fn handle_request(
        ...
        // Version checking happens before dispatch so every operation shares the
    ...
    fn list_tasks(
        ...
        // list_tasks reads persisted authority state and the latest queue
        // projection, but it never calls mutation service or bumps revisions.
        if !request.status.is_empty() {
            tasks.retain(|task| request.status.contains(&task.status));
        }
        // Newest updates first makes the tool useful for deciding whether a
        // recent task already covers the worker's intended follow-up.
    ...
    fn create_task(
        ...
        if !request.apply {
            // The prompt asks workers to list first; apply=false is rejected
            // rather than silently previewing a task that cannot be committed.
            return Err(anyhow!(
                "create_task requires apply=true; run list_tasks first if you need context"
            ));
        }
        // Source is always Llm because this API is invoked by the planning
        // worker tool, while source_turn_id preserves the surrounding turn.
        let mutation = self
            .task_mutation_service
    ...
    fn update_task(
        ...
        if !request.apply {
            // Updates are authority writes, so the same explicit-apply guard is
            // used for partial patches as for creates.
            return Err(anyhow!(
...
impl From<PlanningTaskCreatePayload> for PlanningTaskCreateInput {
    fn from(input: PlanningTaskCreatePayload) -> Self {
        // Keep conversion lossless; defaults such as status/priority are owned
        // by PlanningTaskMutationService so tool and fallback commands agree.
        Self {
...
impl From<PlanningTaskUpdatePayload> for PlanningTaskUpdateInput {
    fn from(input: PlanningTaskUpdatePayload) -> Self {
        // Option<Vec<_>> is preserved here so mutation can distinguish omitted
        // relation fields from an explicit dependency clear.
        Self {
...
pub fn planning_task_tool_contract_json() -> &'static str {
    // The contract is embedded in prompts and CLI output, so it stays compact
    // and stable rather than being generated from Rust types at runtime.
    TASK_TOOL_CONTRACT_JSON
}
fn validate_version(version: u32) -> Result<()> {
    // Tool JSON is versioned independently from planning authority documents.
    // This keeps request compatibility explicit when the command schema changes.
    if version == 1 {
        Ok(())
    } else {
        Err(anyhow!(
...
fn response_from_mutation(
    ...
    // Mutation responses intentionally omit a task list.  Callers should perform
    // a fresh list_tasks call if they need a full post-commit view.
    PlanningTaskToolResponse {
        ok: true,
...
mod tests {
    ...
    fn contract_is_compact_and_names_run_command() {
        // The contract must fit in prompts while still steering workers away
        // from file edits and payload.worktree_path misuse.
    ...
    fn create_task_request_is_flat_for_llm_use() {
        // Flat JSON is part of the model-facing ergonomics: if this starts
        // requiring a nested task object, prompt examples become harder to use.
