...
| 11 | 턴 예산 에디터 셸 명령 무효 커밋 | 에디터가 열린 상태에서 `:turns garbage` 같은 무효 값을 제출하면 상태 메시지는 "unchanged"를 보여주지만 인라인 편집 버퍼가 열린 채 남는 문제를 확인. 무효 커밋 분기에서도 `EditFinished`를 디스패치해 버퍼를 닫도록 수정하고(`app_runtime.rs` MaxAutoTurnsUpdated), 유효/무효 경로 모두 정책 유지·버퍼 종료를 검증하는 회귀 테스트(`turn_budget_editor_invalid_commit_keeps_policy_and_closes_draft_via_shell_command`)를 추가. | 수정 완료 — 무효 커밋 시 버퍼 None으로 종료, 정책은 off 유지 |
