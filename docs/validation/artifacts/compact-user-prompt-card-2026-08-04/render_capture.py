...
draw.rectangle((0, y(0) - 2, width, y(1) - 2), fill=USER_PROMPT_BACKGROUND)
...
draw_text(draw, 0, 1, "›", BRAND)
for row in (2, 7):
    draw_text(draw, row, 0, "Codex Commentary:", BRAND)
draw_substring(draw, lines, 5, "› ◆ explore", MAGENTA)
draw_substring(draw, lines, 10, "▼ ◆ patch", MAGENTA)
draw_substring(draw, lines, 11, "[update]", ACCENT)
draw_text(draw, 12, 0, lines[12], MUTED)
draw_text(draw, 13, 0, lines[13], RED)
draw_text(draw, 14, 0, lines[14], GREEN)
draw_text(draw, 24, 0, "Akra", BRAND)
draw_substring(draw, lines, 24, "PENDING", AMBER)
draw_text(draw, 25, 0, "startup:", MUTED)
draw_text(draw, 26, 0, "… CHECKING", AMBER)
...
image.save(HERE / "compact-user-prompt-card-100x30.png")
