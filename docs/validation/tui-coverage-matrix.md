...
- Production shell renderers consume one owned `InlineShellFrameModel`, return an
  `InlineFrameRenderReceipt`, and may mutate only Ratatui `Frame`. Architecture guards exclude
  live app/Core/application/control-plane/service/I/O access while ignoring `#[cfg(test)]`
  fixtures.
...
- Session overlay presentation must consume one screen model captured before draw; filtering,
  paging, and selection repair run once, and repeated redraws must perform no session-catalog I/O.
...
Option A remains the shipped default Ratatui/Crossterm structure. This matrix materializes the
release-blocking proof shape for that stack. The
...
| I6 | Automatic newline fallback preserves host history, viewport, and cursor; standard mode remains explicit diagnostic coverage | release-blocking for automatic; representative for explicit standard | release-blocking for automatic; representative for explicit standard | release-blocking for automatic; representative for explicit standard | release-blocking for automatic; representative for explicit standard | `TestBackend` + vt100 | yes if insertion strategy or escape-sequence behavior changed |
...
| I4 | applicable after switch if host history flush exists | applicable after switch if replay viewport state exists | branch relevance only if reset path touches standard insertion | branch relevance only if reset path touches fallback insertion | `inline_terminal_adapter/tests.rs`: `conversation_identity_switch_replays_shared_prefix_once_without_loading_frame`, `viewport_replay_sync_skips_host_scrollback_insertions`; `shell_runtime/tests/input.rs`, flow tests, conversation/runtime entrypoints listed in this matrix |
...
| I6 | primary where host history uses automatic insertion | branch relevance only if replay path still exercises insertion semantics | representative when explicitly selected | primary default | `history_insertion.rs` tests for standard/newline fallback, wide-char, wrapped suffix, cursor restore; vt100 fallback regressions in `inline_terminal_adapter/tests.rs` |
...
- Manual terminal capture stays primitive-sensitive only and does not authorize broader Option B
  extraction.
...
| --- | --- | --- | --- | --- |
| Unified Work Center | `src/adapter/inbound/tui/app/work_center_overlay_ui.rs`, `app/inline_shell_commands.rs`, `app/inline_frame_model.rs`, `src/adapter/inbound/tui/app/shell_presentation/overlays/popup/work_center.rs`, `app/shell_rendering/inline_inspection.rs`, `shell_chrome.rs` | `src/adapter/inbound/tui/app/work_center_overlay_ui.rs`, `app/inline_shell_commands/tests.rs`, `app/shell_controller.rs`, `src/adapter/inbound/tui/app/shell_presentation/overlays/popup/work_center.rs`, `app/shell_rendering_tests.rs`, `app/shell_runtime/tests/input.rs`, `shell_chrome.rs` | The read-only `:work` surface composes existing task, agent, terminal, approval, and delivery authorities into five stable rows while owning only a local selection cursor. Pure projection tests cover idle, running, blocked, and `DESYNC`; full-frame assertions and snapshots cover 80/120/160 columns, direct drill-in keys, resize, and close-to-prompt focus restoration. Interrupting runtime approval suspends the center and returns to it after resolution. | Keep new authority summaries explicit about `UNKNOWN` and reuse the existing drill-down surface instead of adding shadow state. |
| Inline terminal, host scrollback, viewport, resize, redraw transaction | `app/inline_terminal_adapter/**`, `app/history_insertion.rs` | `app/inline_terminal_adapter/tests.rs`, `app/inline_terminal_adapter/tests/history_flush.rs`, `app/history_insertion.rs` | Host scrollback insertions, viewport replay, resize, focus-reacquire visible-buffer rebuild, redraw cache invalidation, fallback insertion, vt100 terminal behavior, and direct frame-recorder regressions. Owned render receipts apply once only after a stable draw; failure, resize race, stale attempt, duplicate attempt, or changed compare-and-apply baseline applies nothing. | Keep new temporal bugs in `InlineFrameRecorder`; add width-specific vt100 cases when terminal escape behavior changes. |
| Parallel event stream, live-tail, prompt position, command hints | `app/parallel_*`, `app/parallel_mode/**`, `app/shell_presentation/overlays/popup/supersession/**`, `app/shell_presentation/status_panels/**`, `supersession_mud.rs` | `app/parallel_terminal_delivery.rs`, `app/inline_terminal_adapter/tests.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_tests.rs`, `app/shell_runtime/tests/flows.rs`, `app/shell_runtime/tests/input.rs`, `app/parallel_peek_overlay_ui.rs`, domain/application parallel supervisor tests | The focused 16-row Parallel Operations board has 80/120/160-column golden snapshots, exact pool/roster/session/distributor joins, `DESYNC` and unknown truth, typed delivery gates, accepted-queue separation, identity-stable refresh selection, `V` agent-view routing, and focused composer ownership. One canonical event window feeds a generation-qualified monotonic host frontier and disjoint typed host/live models; host, frame, and conversation receipts settle independently. The newest undelivered event remains live under clipping, focused frames anchor the hidden cursor at the live viewport origin, and durable batches carry resize guards. Event rows survive focus reacquire, repeated focused/narrow/passive/closed/reopened resize cycles, and long wrapped identity errors; split scrollback/live-tail streams render as a titleless live tail and panel chrome stays out of host scrollback. | Add frame-recorder tests for any new parallel live panel, title visibility, stream offset, command-hint row, or lane-detail row that can move between scrollback and live viewport. |
| Overlay surfaces: help, session, planning, model/view/language selection, parallel peek, progressive activity | `app/language.rs`, `app/*_overlay_ui.rs`, `app/directions_maintenance_ui.rs`, `app/progressive_activity_overlay_ui.rs`, `app/queue_overlay_controller.rs`, `app/queue_overlay_ui.rs`, `app/planning/**`, `app/planning_*`, `app/session_overlay_screen_model.rs`, `app/session_overlay_ui.rs`, `app/shell_presentation/overlays/**`, `app/shell_presentation/overlays/activity.rs`, `shell_chrome.rs` | `app/directions_maintenance_ui.rs`, `app/inline_terminal_adapter/tests.rs`, `app/shell_controller.rs`, `app/shell_presentation/overlays/activity.rs`, `app/shell_presentation/overlays/activity_diff/tests.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_contract_tests/planning.rs`, `app/shell_rendering_tests.rs`, `app/language.rs`, `app/planning_draft_editor_ui/tests.rs`, `app/planning/controller.rs`, `app/planning_workspace_operation_ui.rs`, `app/progressive_activity_overlay_ui.rs`, `app/queue_overlay_ui.rs`, `app/reviews_overlay_ui.rs`, `app/session_overlay_screen_model.rs`, `app/session_overlay_ui.rs`, `app/model_selection_overlay_ui.rs`, `app/view_selection_overlay_ui.rs`, `shell_chrome.rs` | Popup-free inline inspections, overlay focus ownership, planning editor close guards, session browser state, model/view/language pickers, queue selection and cancellation, conversation-tail queue receipt undo, queue/planning snapshots, and bounded Diff/Output pages with resize, approval-priority, ANSI, and host-scrollback assertions. The session overlay owns one page-scoped immutable screen model captured before draw; list/detail/warning/key sections and frame-local list state share it, stale samples remain coherent, repeated draw performs no catalog I/O, and only a stable receipt may compare-and-apply feedback. Review Center, Planning Queue, and Directions Maintenance authority load into request-correlated screen snapshots; presentation consumes those immutable models; repeated redraws perform no application or repository I/O. Directions loading additionally proves non-blocking open, latest-request correlation, workspace-drift rejection, and late-completion rejection after close. The shared planning-workspace coordinator proves reset, simple stage/load/promotion, planning/directions editor staging, and all four editor save/promote entry points use non-blocking dispatch, full target/session/buffer-revision identity, exact duplicate coalescing, busy exclusion, zero provider I/O for malformed identity, one existing action use case, redacted panic completion, stale/duplicate/ABA/wrong-kind rejection, close/workspace/approval/new-session drift rejection, coalesced presentation-revision rebinding, in-flight editing, A→B→A promotion refresh, workspace-B no-refresh, save without pause/refresh, and exact promotion authority refresh without replacing newer status, body, dirty state, or overlay intent. Rust-aware AST guards reject every production TUI direct save/promote callable form and `.workspace()` exposure. | Add pure projection tests when a new overlay section builder appears before relying on a full-frame snapshot; keep real-terminal capture with the primitive-sensitive validation lane. |
| Startup diagnostics, operator ribbon, command palette | `app/inline_frame_model.rs`, `app/language.rs`, `app/shell_controller.rs`, `app/shell_presentation/overlays/popup/base.rs`, `app/shell_presentation/prompt_composer.rs`, `app/shell_presentation/status_panels/operator_ribbon.rs`, `app/shell_rendering/inline_inspection.rs` | `app/shell_controller.rs`, `app/shell_presentation/status_panels/operator_ribbon.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_tests.rs` | Startup diagnostics deduplicate equivalent startup/operator warnings and own a receipt-correlated warning scroll cursor with row, page, and bounds navigation. Modal chrome suppresses the global `Ctrl+D` hint while that action is unavailable. Command-palette field labels follow the selected language at compact and wide widths. | Preserve modal action truth and add localized width assertions when palette fields or diagnostics navigation change. |
...
| Shell rendering snapshots plus targeted assertions | `app/inline_frame_model.rs`, `app/shell_rendering.rs`, `app/shell_rendering/**`, `app/shell_layout.rs`, `app/shell_presentation.rs`, `app/shell_presentation/**`, `app/theme.rs`, `conversation_text.rs` | `app/shell_rendering_tests.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_contract_tests/planning.rs`, `app/snapshots/**` | Ready shell, streaming shell, viewport replay, queue/planning overlays, Parallel Operations at 80/120/160 columns, inline inspections, queue undo mouse hit targets, typed render surface routing, narrow terminal behavior, border-free inline layout, cursor placement, one immutable conversation tail projection, and modal cursor hide/exact restore. Pre-draw capture owns one `InlineShellFrameModel`/active `InlineInspectionFrameModel`; production renderers receive no live app or authority and return one `InlineFrameRenderReceipt`. Rust-aware guards scan the production renderer tree while excluding test fixtures. | When updating a snapshot, add or update a nearby assertion that names the regression the snapshot should catch. |
| vt100 terminal path | `app/tui_testkit.rs`, `app/history_insertion.rs`, `app/inline_terminal_adapter/**`, `app/shell_rendering_tests.rs` | `app/tui_testkit.rs`, `app/history_insertion.rs`, `app/inline_terminal_adapter/tests.rs`, `app/shell_rendering_tests.rs` | ANSI/cursor/clear output, automatic newline fallback, explicit standard insertion, host scrollback rows, visible screen text, wrapping, repeated resize, durable boundary guards, and vt100 shell snapshots. | Add vt100 tests for any new escape sequence, terminal clear path, or cursor-sensitive prompt movement. |
| Startup, session, conversation, auto-follow, planning control state | `app.rs`, `app/app_runtime.rs`, `app/auto_follow/**`, `app/auto_follow_controls.rs`, `app/conversation/**`, `app/conversation_*`, `app/github_polling/**`, `app/post_turn_continuation.rs`, `app/turn_submission_runtime/**` | `app.rs`, `app/app_runtime.rs`, `app/auto_follow_controls.rs`, `app/auto_follow_overlay_ui.rs`, `app/conversation_model_tests.rs`, `app/conversation_runtime.rs`, `app/github_polling/tests.rs`, `app/inline_terminal_adapter/tests.rs`, `app/shell_runtime/tests.rs`, `app/turn_submission_runtime.rs`, `app/shell_entrypoint.rs`; Core controller/effect-runner tests and `tests/architecture_boundaries.rs` | App state transitions, runtime effects, conversation lifecycle, auto-follow controls, turn submission, and manual prompt preparation. Post-turn panel start is Core-owned with latest confirmed-terminal/exact in-flight admission, stale/wrong/duplicate rejection, exact pause → protected-file repair → empty-stop preserve → refresh priority, full summary/detail preservation, one identical Started-event/effect state, and Started-before-immediate-completion ordering. Its monotonic correlation binds generation, thread, turn, turn workspace, and planning workspace across effect/completion; leaving the lifecycle prunes the lease even without another evaluation admission, A→B→A and duplicate completions cannot clear a newer lease, nested provenance/receipt identity is rechecked, and malformed worker output becomes one correlated safe failure without mutating a newer continuation permit. The TUI applies the state immediately without raw application/planning handles or direct workspace/runtime use-case exposure. Startup proves a 650 ms gated prompt-log maintenance port cannot block build/prepare or first-draw eligibility; exact generation/workspace correlation rejects stale A→B→A, mismatched, duplicate, and redacted panic completions. GitHub setup proves zero dispatch on failed draw, one dispatch after the first stable delivered frame, a sub-300ms first frame while a 650 ms provider is blocked, live input/redraw during setup, exact generation/workspace service binding, A→B→A/stale/duplicate/panic fail-closed behavior, and setup completion before polling. Rust-aware guards exclude comments, strings, and test-only items while banning direct post-turn helper callable forms, raw TUI application/planning handles, prompt-log maintenance, and GitHub provider/process ownership in production TUI paths. | Add reducer tests before introducing new runtime state that can affect render output; preserve the first-frame timing and exact-correlation setup proofs when changing bootstrap or terminal delivery. |
