...
| Overlay surfaces: help, session, planning, model/view/language selection, parallel peek, progressive activity | `app/language.rs`, `app/*_overlay_ui.rs`, `app/progressive_activity_overlay_ui.rs`, `app/queue_overlay_ui.rs`, `app/planning/**`, `app/planning_*`, `app/session_overlay_ui.rs`, `app/shell_presentation/overlays/**`, `app/shell_presentation/overlays/activity.rs`, `shell_chrome.rs` | `app/inline_terminal_adapter/tests.rs`, `app/shell_presentation/overlays/activity.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_contract_tests/planning.rs`, `app/shell_rendering_tests.rs`, `app/language.rs`, `app/planning_draft_editor_ui/tests.rs`, `app/planning/controller.rs`, `app/progressive_activity_overlay_ui.rs`, `app/queue_overlay_ui.rs`, `app/reviews_overlay_ui.rs`, `app/session_overlay_ui.rs`, `app/model_selection_overlay_ui.rs`, `app/view_selection_overlay_ui.rs`, `shell_chrome.rs` | Popup-free inline inspections, overlay focus ownership, planning editor close guards, session browser state, model/view/language pickers, queue selection and cancellation, conversation-tail queue receipt undo, queue/planning snapshots, and bounded Diff/Output pages with resize, approval-priority, ANSI, and host-scrollback assertions. Review Center authority loads into a request-correlated workspace/thread screen snapshot, presentation consumes that immutable model, and repeated redraws perform no application or repository I/O. | Add pure projection tests when a new overlay section builder appears before relying on a full-frame snapshot; keep real-terminal capture with the primitive-sensitive validation lane. |
