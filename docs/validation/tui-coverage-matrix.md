...
| Transcript selection and clipboard | `app/transcript_viewport_ui.rs`, `app/terminal_interaction_ui.rs`, `app/ratatui_frontend.rs` | forward/reverse selection, rendered-cell highlight, OSC 52/tmux, unconditional mouse lifecycle | Stable frame cells are selectable; app-owned pointer input is a fullscreen invariant. |
| Terminal input admission and frame cadence | `composition/native_terminal_event_ingress.rs`, `app/ratatui_frontend.rs`, `app/shell_runtime.rs` | ingress mailbox, motion filtering, drag-burst reduction, scheduler tests, E3 PTY capture | Input drains during frame writes; unused motion is dropped; one latest frame is admitted per 16ms boundary. |
...
| F8 | Semantic interaction zones and continuous input admission prevent stale drag replay | input ingress, selection ranges, or frame cadence changes |
