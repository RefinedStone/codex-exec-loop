...
| Parallel Operations, event stream, live-tail, prompt position, command hints | `app/parallel_*`, `app/parallel_mode/**`, `app/shell_presentation/overlays/popup/supersession/**`, `app/shell_presentation/status_panels/**`, `supersession_mud.rs` | `app/inline_terminal_adapter/tests.rs`, `app/shell_rendering_contract_tests.rs`, `app/shell_rendering_tests.rs`, `app/shell_runtime/tests/flows.rs`, `app/shell_runtime/tests/input.rs`, `app/parallel_peek_overlay_ui.rs`, domain/application parallel supervisor tests | The focused 16-row board has 80/120/160-column golden snapshots, exact pool/roster/session/distributor joins, `DESYNC` and unknown truth, typed delivery gates, accepted-queue separation, identity-stable refresh selection, `V` agent-view routing, and focused composer ownership. Event stream rows survive focused → narrow → passive resize/close sequences in `focused_parallel_operations_survives_resize_and_close_without_chrome_in_scrollback`; split scrollback/live-tail streams render as a titleless live tail and panel chrome stays out of host scrollback. | Add frame-recorder tests for any new parallel live panel, title visibility, stream offset, command-hint row, or lane-detail row that can move between scrollback and live viewport. |
