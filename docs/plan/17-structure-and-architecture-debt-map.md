...
- `src/application/service/planning/runtime/policy.rs` keeps auto-follow policy and status
  projection contract tests in `runtime/policy/tests.rs`.
...
- `src/application/service/parallel_mode/mod.rs` keeps agent branch naming, slug truncation, and
  ...
  `parallel_mode/orchestration.rs`, slot lease lifecycle transitions in
  `parallel_mode/slot_lifecycle.rs`, and shared git/fs support helpers in
...
- `src/adapter/inbound/tui/app/inline_terminal_adapter.rs` keeps ratatui backend wrapping and
  cursor tracking in `inline_terminal_adapter/backend.rs`, and host-history flush tracking in
  `inline_terminal_adapter/history_flush.rs`.
...
- `src/adapter/inbound/admin_api/mod.rs` keeps JSON planning API handlers in
  `admin_api/api.rs`, browser page handlers in `admin_api/pages.rs`, and CSRF/render/redirect
  helpers in `admin_api/helpers.rs`, leaving server bootstrap and router wiring in the parent
  module.
- `src/adapter/inbound/cli.rs` keeps doctor/init/reset/planning-tool report DTOs and renderers in
