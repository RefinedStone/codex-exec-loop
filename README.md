...
| `:turns <n|infinite>` | 최대 auto follow-up 횟수 설정 |
| `:new` | 새 draft 열기 |
| `:help` | 사용 가능한 셸 명령 표시 |

지원 alias도 있습니다. 예를 들어 `:diagnostics`, `:session`, `:template`, `:q`, `:planning-init`, `:auto-turns 10`, `:turns infinite`도 동작합니다. `:directions`와 `:stop`은 단축 alias 없이 그대로 사용합니다.
