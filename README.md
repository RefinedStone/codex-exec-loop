...
| `:doctor` | planning health 요약 보기 |
| `:init` | 기본 planning scaffold review 열기 |
| `:reset <queue|directions|all>` | planning 상태 reset |
...
지원 alias도 있습니다. 예를 들어 `:diagnostics`, `:session`, `:automation`, `:q`, `:turn 10`, `:auto-turns 10`도 동작합니다. `:directions`, `:doctor`, `:init`, `:planning`, `:reset`, `:stop`은 그대로 사용합니다.
...
- `:planning`: planning controls와 기존 workspace 상태를 엽니다.
- `:init`: 빈 workspace에서는 기본 simple scaffold review로 바로 들어갑니다.
- `:doctor`: 현재 planning health, queue 요약, queue-idle 상태를 compact하게 봅니다.
...
planning lifecycle 명령:

- `akra doctor [workspace_dir]`: planning 상태를 read-only로 검사하고 `absent`, `incomplete`, `invalid`, `ready_without_task`, `ready_with_task` 중 하나를 출력합니다.
- `akra init [workspace_dir]`: 기본 simple scaffold를 active planning 파일로 바로 생성합니다. 기존 workspace가 있으면 실패합니다.
- `akra reset <queue|directions|all> [workspace_dir]`: queue 쪽만, directions 쪽만, 또는 전체 scaffold를 reset합니다.
- `:doctor`: 셸 안에서 같은 planning health 요약을 보여줍니다.
- `:init`: 빈 workspace에서는 simple review를 열고, 이미 초기화된 workspace에서는 기존 controls로 안내합니다.
- `:reset queue|directions|all`: 같은 reset semantics를 셸에서 실행합니다. `directions`와 `all`은 `confirm`이 필요합니다.
