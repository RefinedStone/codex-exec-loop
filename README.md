...
지원 alias도 있습니다. 예를 들어 `:diagnostics`, `:session`, `:template`, `:q`, `:planning-init`, `:auto-turns 10`도 동작합니다. `:stop`은 단축 alias 없이 그대로 사용합니다.
...
현재 queue 상태를 빠르게 확인하려면 `:queue` 또는 `:q`를 사용하세요. 이 화면은 raw planning JSON 전체를 덤프하지 않고, 다음 실행 후보와 proposal, skipped 요약만 사람이 읽기 쉬운 형태로 보여줍니다.
