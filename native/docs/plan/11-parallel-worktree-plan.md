...
- status: completed on `prerelease`
...
- landed follow-ups inside A6 so far:
  ...
  - active prompt cursor ownership is visible in inline mode
  - live agent output streams before completion
  - routine status no longer prints raw turn ids
  - hidden inline mode now uses a fixed viewport budget and bottom-anchored tail instead of rebuilding viewport height as the tail grows or shrinks
- remaining note:
  - only platform validation remains; new code work should start only if real terminal testing still shows replay-like behavior
