...
- Compare the latest operator request and main-session result against `[accepted-db-direction-authority]` goals, success criteria, and detail doc paths plus `[accepted-db-task-authority]` and `[db-queue-projection]`.
...
- Ignore older prompt or direction wording that treats file-backed planning authority or answer-implied completion as the completion test; accepted DB authority and evaluator judgment win. Do not rerun the whole project or duplicate completed work unless DB authority itself proves work remains.
